...
use Entrada\Modules\Olab\Models\H5pContentsLibraries;
use Entrada\Modules\Olab\Models\H5pLibraries;
use Entrada\Modules\Olab\Models\H5pLibrariesLibraries;
use Entrada\Modules\Olab\Models\H5pLibrariesLanguages;
use Entrada\Modules\Olab\Models\H5pCounters;
use Entrada\Modules\Olab\Models\H5pLibrariesCachedassets;
...
class H5POpenLabyrinth implements H5PFrameworkInterface
{
    ...
    {
      $record = H5pLibrariesLibraries::where('library_id', $id);
      $record->delete();
    ...
    public function deleteLibrary($library)
    {
      ...
      H5PCore::deleteFileTree($this->getH5pPath() . 
                              '/libraries/' . $library->name . '-' . 
                              $library->major_version . '.' . $library->minor_version);

      $record = H5pLibrariesLibraries::where('library_id', $library->id);
      $record->delete();

      $record = H5pLibrariesLanguages::where('library_id', $library->id);
      $record->delete();

      $record = H5pLibraries::where('library_id', $library->id);
      $record->delete();

      // Remove library data from database
      //$wpdb->delete($wpdb->prefix . 'h5p_libraries_libraries', 
      //              array('library_id' => $library->id), array('%d'));
      //$wpdb->delete($wpdb->prefix . 'h5p_libraries_languages', 
      //              array('library_id' => $library->id), array('%d'));
      //$wpdb->delete($wpdb->prefix . 'h5p_libraries', 
      //              array('id' => $library->id), array('%d'));
    }
    ...
    {
      $record = H5pContentsLibraries::where('content_id',$contentId);
      $record->delete();
    ...
    public function saveLibraryUsage($contentId, $librariesInUse)
    {
        ...
        foreach ($librariesInUse as $dependency) {
            ...
            $contentLibrary = new H5pContentsLibraries;
            $contentLibrary->content_id = $contentId;
            $contentLibrary->library_id = $dependency['library']['libraryId'];
            $contentLibrary->dependency_type = $dependency['type'];
            $contentLibrary->drop_css = $dropCss;
            $contentLibrary->weight = $dependency['weight'];
    ...
    public function loadLibrary($name, $majorVersion, $minorVersion)
    {
        ...
        $library = H5pLibraries::Get( $name, $majorVersion, $minorVersion );
        ...
        $dependencies = H5pLibrariesLibraries::GetByLibrary( $library['libraryId'] );
    ...
    public function loadLibrarySemantics($name, $majorVersion, $minorVersion)
    {
        ...
        if ($this->isInDevMode()) {
            ...
            $libraryRecord = H5pLibraries::Get( $name, $majorVersion, $minorVersion );

            $semantics = $libraryRecord['semantics'];
    ...
    {
      $content = H5pContents::LoadContent( $id );
      return $content;
    ...
    {
      $content = H5pContentsLibraries::LoadContentDependencies( $id, $type );
      return $content;
    ...
    public function getOption($name, $default = false)
    {
        ...
        return OLabUtilities::get_option('h5p_' . $name, $default);
    }
    ...
    public function isContentSlugAvailable($slug)
    {
        ...
        return !$wpdb->get_var($wpdb->prepare(
          "SELECT slug FROM {$wpdb->prefix}h5p_contents WHERE slug = '%s'", $slug));
    }
    ...
    public function getLibraryContentCount()
    {
        ...
        $results = H5pContents::LibraryContentCount();
    ...
    public function getLibraryStats($type)
    {
        ...
        $results = H5pCounters::LibraryStats( $type );
    ...
    {
      return H5pContents::GetAuthorCount();
    ...
    public function saveCachedAssets($key, $libraries)
    {
        ...
        foreach ($libraries as $library) {
            ...
            $already_exists = H5pLibrariesCachedassets::Exists( $id, $key );
            ...
            if (!$already_exists) {
              ...
              $rec = new H5pLibrariesCachedassets();
    ...
    public function deleteCachedAssets($library_id)
    {
        ...
        $results = H5pLibrariesCachedassets::ByLibraryId( $library_id );
