...
class H5PPluginAdmin
{
    ...
    public function admin_notices()
    {
        ...
        $update_available = OLabUtilities::get_option('h5p_update_available', 0);
        $current_update = OLabUtilities::get_option('h5p_current_update', 0);
        if ($current_update == 0) {
        ...
        $last_print = OLabUtilities::get_option('h5p_last_info_print', 0);
    ...
    {
        $url = OLabUtilities::get_option('h5p_update_available_path', null);
        if (!$url) {
        ...
        if (is_wp_error($response)) {
            ...
            if ($validator->isValidPackage(true, $update_only)) {
                ...
                update_option('h5p_current_update', OLabUtilities::get_option('h5p_update_available', 0));
    ...
    public static function check_upload_sizes($dir)
    {
        ...
        foreach ($files as $file) {
            ...
            $path = OLabUtilities::concat_path($dir, $file);
            if (is_dir($path)) {
                // Scan dir
    ...
    public function print_results($content_id = null, $user_id = null)
    {
        ...
        $datetimeformat = OLabUtilities::get_option('date_format') . ' ' . OLabUtilities::get_option('time_format');
        $offset = OLabUtilities::get_option('gmt_offset') * 3600;
