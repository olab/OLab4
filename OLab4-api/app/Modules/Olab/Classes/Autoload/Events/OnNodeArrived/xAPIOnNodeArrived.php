 ...
 * @author wirunc
 */
class xAPIOnNodeArrived implements iEventHandler
{
    ...
    public function fireEvent(...$event_args )
    {
        ...
        {            
            ...
            // only generate a statement if xAPI enabled on map.
            if ( $oMap->send_xapi_statements ) {
                $this->buildOnNodeArrived( $oState, $oMap, $oNode );
            }
     ...
     * @param UserState $session_id 
     * @param Maps $oMap 
     * @param MapNodes $oNode 
     */
    private function buildOnNodeArrived( UserState $oState, Maps $oMap, MapNodes $oNode )
    {
        $timestamp = time();
        ...
        $xapi_object = $xapi_node->toxAPIObject();
