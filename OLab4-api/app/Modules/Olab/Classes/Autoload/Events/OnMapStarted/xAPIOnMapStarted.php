 ...
 * @author wirunc
 */
class xAPIOnMapStarted implements iEventHandler
{
     ...
     * @param array $event_args ([0][0][0] = user state, [0][0][1] = MapNode)
     */
    public function fireEvent(...$event_args )
    {
        ...
        try
        {
            ...
            if ( ( $oMap->send_xapi_statements ) && 
                 ( $oNode->type_id == MapNodeTypes::RootTypeId() ) ){
                $this->buildOnSessionInitialized( $oState, $oMap, $oNode );
            }
        }
     ...
     */
    private function buildOnSessionInitialized( UserState $oState, Maps $oMap, MapNodes $oNode )
    {
        $tracer = new OlabCodeTracer(__CLASS__, __FUNCTION__ . "()" );
        ...
        $oMainCounter = Counters::MapMainCounter( $oMap->id )->first();
        ...
        $context['extensions'][xAPIStatement::getExtensionNodeKey()] 
            = $xapi_node->toxAPIExtensionObject();
