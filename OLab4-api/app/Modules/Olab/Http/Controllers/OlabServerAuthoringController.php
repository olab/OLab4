...
class OlabServerAuthoringController extends OlabAuthoringController
{
  // override for abstract class to provide url subpath
  public function get_object_url_subpath() { return "servers"; }

  protected function get_deep_server( $id ) {
    
    $oObj = Servers::with( ScopedObjectManager::RELATION_NAME_COUNTERS )
              ->with( ScopedObjectManager::RELATION_NAME_CONSTANTS )
              ->with( ScopedObjectManager::RELATION_NAME_QUESTIONS )
              ->find($id);

    if ( $oObj == null ) {
      throw new OlabObjectNotFoundException("Server", $id );
    }
  ...
  public function getSingle( Request $request, $id ) {
    ...
    try {
      ...
      $oObj = $this->get_deep_server( $id );

      $aObj = $oObj->toArray();

      $payload = OLabUtilities::make_api_return( null, $tracer, $aObj );
