	...
	jQuery(document).ready(function($){
		...
		*/
		...
		jQuery('#evaluation-topics-toggle').trigger('click');
	...
	function unmapObjective(id,list,importance){
		...
		var children_exist = jQuery("#mapped_evaluation_objectives li").length;
	...
	function mapObjective(id,title,description,list,create){
		...
		if(create && lkey == -1 && key == -1){
			...
			var controls = 	jQuery(document.createElement('div'))
								.attr('class','evaluation-objective-controls');
			...
			//jQuery(li).append(rm);			
			jQuery('#mapped_evaluation_objectives').append(li);
			jQuery('#mapped_evaluation_objectives .display-notice').remove();
			jQuery('#objective_'+id).parents('.objective-list').each(function(){
				var id = jQuery(this).attr('data-id');
				jQuery('#check_objective_'+id).prop('checked',true);
				jQuery('#check_objective_'+id).prop('disabled',true);
			});		
			if(jQuery('#evaluation-toggle').hasClass('collapsed')){
				jQuery('#evaluation-toggle').removeClass('collapsed');
				jQuery('#evaluation-toggle').addClass('expanded');
				var d = jQuery('#evaluation-toggle').next();
				jQuery(d).slideDown();
			}
			if(!jQuery('#evaluation-list-wrapper').is(':visible')){
				jQuery('#evaluation-list-wrapper').show();
			}	
			list = 'evaluation';
		}
