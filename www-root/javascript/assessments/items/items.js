...
function get_items(item_index, specified_width) {
    ...
    jQuery.when(items).done(function (data) {
        ...
        if (jsonResponse.results > 0) {
            ...
            var replacement_string = items_localization.load_more_template;
            replacement_string = replacement_string.replace("%%current_total_items%%", total_items);
            replacement_string = replacement_string.replace("%%total_items%%", jsonResponse.data.total_items);
            jQuery("#load-items").html(replacement_string);

            item_offset = (item_limit + item_offset);

            // ADRIAN-TODO: This needs some reworking
...
function build_item_additional_details(item_id) {
    ...
    jQuery.when(details).done(function (data) {
        ...
        switch (jsonResponse.status) {
            case "success" :
                // TODO: Move this to a loadTemplate compatible view
                ...
                // TODO: Add localization (should be done within the loadTemplate compatible view)
                var comment_type;
...
jQuery(document).ready(function ($) {
    ...
    jQuery(".view-toggle").on("click", function (e) {
        ...
        if (selected_view === "list") {
            if (jQuery("#items-table tbody").children().length > 0) {
                jQuery("#items-table").removeClass("hide");
            }
    ...
    // ADRIAN-TODO: This needs refactoring
