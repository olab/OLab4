        ...
        case "GET" :
            if(isset($request["method"])) {
                switch ($request["method"]) {
                    case "get-schedule-objectives":
                        if (isset($request["schedule_parent_id"]) && $tmp_input = clean_input($request["schedule_parent_id"], array("trim", "int"))) {
                            $PROCESSED["schedule_parent_id"] = $tmp_input;
                        } else {
                            add_error($translate->_("No schedule ID provided"));
                        }
                        if (!$ERROR) {
                            $objectives = array();
                            $data = "No Objectives Found";
                            $status = "error";
                            if ($PROCESSED["schedule_parent_id"]) {
                                $schedule = Models_Schedule::fetchRowByID($PROCESSED["schedule_parent_id"]);
                                if ($schedule) {
                                    $course_objective_model = new Models_Schedule_CourseObjective();
                                    $objectives[] = $course_objective_model->fetchAllByScheduleIDCourseID($schedule->getCourseID(), $schedule->getScheduleID());
                                }
                            }
                            if (!empty($objectives)) {
                                foreach ($objectives as $objective_group) {
                                    if (!empty($objective_group)) {
                                        $status = "success";
                                        $data = $objective_group;
                                    }
                                }
                            } else {
                                $data = "No Objectives Found";
