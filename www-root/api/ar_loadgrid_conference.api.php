...
if ((isset($_SESSION["isAuthorized"])) && ((bool) $_SESSION["isAuthorized"]) && $ENTRADA_ACL->amIAllowed('annualreport', 'read')) {
    ...
    // this is only for the specific Annual Report table `ar_conference_papers`
    if ($table !== "ar_conference_papers") {
        $error = true;
    }
    ...
    if ($results = $db->GetAll($query, [$proxy_id])) {
        ...
        foreach ($results as $row) {
            ...
            foreach ($fields as $field) {
                ...
                // 'location' column changed to 'city' and 'prov_state' after 2011
                if ($field == "location" && !($row['year_reported'] < '2011')) {
                    $cell[] = $row['city'] . ", " . $row['prov_state'];
                } else {
                    $cell[] = $row[$field];
                }
            }
