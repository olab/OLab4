...
if ($RECORD_ID) {
    $access_file = Models_Quiz_Attached_AccessFiles::fetchRowByAquizID($RECORD_ID);
    ...
    */
    $grant_access = false;
    if ($access_file){
        $access_keys = Models_Quiz_Attached_AccessFileKeys::fetchAllByAquizID($RECORD_ID);
    ...
    if ($grant_access){
                                                    ...
                                                    if ($progress->update()) {
                                                        ...
                                                        */
                                                        if (($quiz_record["quiztype_code"] == "immediate") || (($quiz_record["quiztype_code"] == "delayed") && (((int) $quiz_record["release_until"] === 0) || ($quiz_record["release_until"] <= time())))) {
                                                            header("Location: ".ENTRADA_URL."/secure/quizzes?section=results".(isset($QUIZ_TYPE) && $QUIZ_TYPE == "community_page" ? "&community=true" : "")."&id=".$progress_record["qprogress_id"]);
                                                            exit;
                                                        } else {
                                                            $url = ENTRADA_URL."/secure/secure-logout";
                                                            add_success("Thank you for completing the <strong>".html_encode($quiz_record["quiz_title"])."</strong> quiz. Your responses have been successfully recorded, and your grade and any feedback will be released <strong>".date(DEFAULT_DATE_FORMAT, $quiz_record["release_until"])."</strong>.<br /><br /><p>You may now logout of secure mode.</p><p><a href=\"".$url."\" class=\"btn btn-default\"><strong>Logout</strong></a></p>");
                                        ...
                                        */
                                        ...
                                        if (isset($qprogress_id) && $qprogress_id) {
                                            ...
                                            */
                                            ...
                                            ?>
                                            <form class="question-responses" action="<?php echo ENTRADA_URL."/".$MODULE; ?>/quizzes?section=attempt<?php echo (isset($QUIZ_TYPE) && $QUIZ_TYPE == "community_page" ? "&amp;community=true" : ""); ?>&amp;id=<?php echo $RECORD_ID; ?>" method="post">
                                                                    ...
                                                                    foreach ($grouped_qquestions as $question) {
                                                                        ...
                                                                        if ($responses) {
                                                                            ...
                                                                            foreach ($responses as $r) {
                                                                                ...
                                                                                $quiz_markup .= "	<span class=\"span11\"><label for=\"response_".$question->getQquestionID()."_".$response["qqresponse_id"]."\">".clean_input($response["response_text"], (($response["response_is_html"] == 1) ? "trim" : "encode"))."</label></span>";
                                                                                $quiz_markup .= "</li>\n";
                                                                            }
                                                                            $quiz_markup .= "</ul>";
                                                                        }
                                            ...
                                            <div class="row-fluid border-above-medium space-above pad-top">
                                                <a class="btn btn-default" href="<?php echo ENTRADA_URL; ?>/secure/secure-logout">Exit Quiz</a>
                                            ...
                                            new_sidebar_item("Quiz Statement", $sidebar_html, "page-anchors", "open");
                                        } else {
       ...
       add_error("You must use an approved version of Safe Exam Browser to attempt this quiz.");
       echo display_error();
       application_log("error", "The SEB header did not match any of the keys added for this quiz");
   }
} else {
	add_error("In order to attempt a quiz, you must provide a valid quiz identifier.");
