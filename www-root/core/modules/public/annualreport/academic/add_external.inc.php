...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["description"]	= "Contributions to External Organisations / International Development Projects portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	$ONLOAD[]					= "setMaxLength();";
	
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/academic?section=add_external", "title" => "Add Contributions to External Organisations / International Development Projects");
	
	// This grid should be expanded upon redirecting back to the academic index.
	$_SESSION["academic_expand_grid"] = "external_grid";
	
	echo "<h1>Add Contributions to External Organisations / International Development Projects</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_external_contributions", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						$SUCCESS++;
						$SUCCESSSTR[]  = "You have successfully added information pertaining to external contribution <strong>".html_encode($PROCESSED["organisation"])."</strong> to the system.<br /><br />".$msg;
						$ONLOAD[]		= "setTimeout('window.location=\\'".$url."\\'', 5000);";

						application_log("success", "Contributions to External Organisations / International Development Projects [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an external contribution record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			$ONLOAD[]		= "provStateFunction(\$F($('addExternalServiceForm')['countries_id']))";
			?>
			...
			<form id="addExternalServiceForm" action="<?php echo ENTRADA_URL; ?>/annualreport/academic?section=add_external&amp;step=2" method="post">
			...
			<tr>
				...
				<td><input type="text" id="organisation" name="organisation" value="<?php echo html_encode($PROCESSED["organisation"]); ?>" maxlength="255" style="width: 95%" /></td>
			</tr>
			<tr>
				<td></td>
			...
			<tr>
				...
				<td><input type="text" id="days_of_year" name="days_of_year" value="<?php echo html_encode($PROCESSED["days_of_year"]); ?>" maxlength="3" style="width: 40px" /></td>
			</tr>
			<tr>
				<td colspan="3">&nbsp;</td>
			</tr>
