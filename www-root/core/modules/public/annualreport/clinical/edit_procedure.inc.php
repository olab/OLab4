...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
    ...
    if (isset($_GET["rid"]) && (int) $_GET["rid"]) {
        $PROCEDURES_ID = clean_input($_GET["rid"], "int");
    } else {
        $PROCEDURES_ID = 0;
    }
		...
		if($result) {
					...
					if(!$ERROR) {
						...
						if($db->AutoExecute("ar_procedures", $PROCESSED, "UPDATE", "`procedures_id`=".$db->qstr($PROCEDURES_ID))) {
								switch($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["post_action"]) {
									case "new" :
										$url = ENTRADA_URL."/annualreport/clinical?section=add_procedure";
										$msg	= "You will now be redirected to add more Procedures; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.";
									break;
									case "index" :
									default :
								...
								application_log("success", "Edited Procedures [".$PROCEDURES_ID."] in the system.");					
		
						} else {
							$ERROR++;
							$ERRORSTR[] = "There was a problem editing this Procedures record in the system. The MEdIT Unit was informed of this error; please try again later.";
		
							application_log("error", "There was an error editing the Procedures. Database said: ".$db->ErrorMsg());
						}
					} else {
						$STEP = 1;
					}
			...
			switch($STEP) {
				...
				default :
					...
					{
						$procedureQuery = "SELECT * FROM `ar_procedures` WHERE `procedures_id` = ?";
						$procedureResult = $db->GetRow($procedureQuery, array($PROCEDURES_ID));
					}
					...
					?>
					<form action="<?php echo ENTRADA_URL; ?>/annualreport/clinical?section=edit_procedure&amp;step=2&amp;rid=<?php echo $PROCEDURES_ID;?>" method="post">
					<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Procedures">
					<colgroup>
						<col style="width: 3%" />
						<col style="width: 20%" />
						<col style="width: 77%" />
					...
					<tr>
						...
						<?php
							...
							foreach($on_callLocationArray as $on_callLocationListValue) {
								echo "<option value=\"".$on_callLocationListValue["on_call_location"]."\"".(($procedureResult["site"] == $on_callLocationListValue["on_call_location"] || $PROCESSED["site"] == $on_callLocationListValue["on_call_location"]) ? " selected=\"selected\"" : "").">".html_encode($on_callLocationListValue["on_call_location"])."</option>\n";
							}
						?>
						</select>
						</td>
					...
					<tr>
						...
						<td><input type="text" id="site_description" name="site_description" value="<?php echo ((isset($procedureResult["site_description"])) ? html_encode($procedureResult["site_description"]) : html_encode($PROCESSED["site_description"])); ?>" style="width: 95%" /></td>
					</tr>
					<tr>
						<td></td>
						<td style="vertical-align: top"><label for="average_hours" class="form-required">Average Hours / Week</label></td>				
						<td><input type="text" id="average_hours" name="average_hours" value="<?php echo ((isset($procedureResult["average_hours"])) ? html_encode($procedureResult["average_hours"]) : html_encode($PROCESSED["average_hours"])); ?>" maxlength="5" style="width: 40px" /></td>
					</tr>
					<tr>
						<td></td>
						<td style="vertical-align: top"><label for="special_features" class="form-required">Special Features</label></td>				
						<td><textarea id="special_features" name="special_features" style="width: 95%" rows="4"><?php echo ((isset($procedureResult["special_features"])) ? html_encode($procedureResult["special_features"]) : html_encode($PROCESSED["special_features"])); ?></textarea></td>
					</tr>
					<tr>
						<td colspan="3">&nbsp;</td>
					</tr>
					<tr>
						...
						if((isset($PROCESSED["year_reported"]) && $PROCESSED["year_reported"] != "")) {
							...
							displayARYearReported($procedureResult["year_reported"], $AR_CUR_YEAR, $AR_PAST_YEARS, $AR_FUTURE_YEARS, false);
						}
						?>
					</tr>
					<tr>
			...
			$ERROR++;
			$ERRORSTR[] = "In order to edit a Procedures record you must provide a valid Procedures identifier. The provided ID does not exist in this system.";

			echo display_error();

			application_log("notice", "Failed to provide a valid Procedures identifer when attempting to edit a Procedures record.");
		}
	} else {
		$ERROR++;
		$ERRORSTR[] = "In order to edit a Procedures record you must provide the Procedures identifier.";

		echo display_error();

		application_log("notice", "Failed to provide Procedures identifer when attempting to edit a Procedures record.");
	}
}
?>