...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["title"]			= "Add Innovation in Clinical Activity";
	$PAGE_META["description"]			= "Innovation in Clinical Activity portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/research?section=add_innovation", "title" => "Add Innovation in Clinical Activity");
	
	// This grid should be expanded upon redirecting back to the clinical index.
	$_SESSION["clinical_expand_grid"] = "innovation_grid";
	
	echo "<h1>Add Innovation in Clinical Activity</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :			
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_clinical_innovation", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						application_log("success", "Innovation in Clinical Activity [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an Innovation in Clinical Activity record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_URL; ?>/annualreport/clinical?section=add_innovation&amp;step=2" method="post">
