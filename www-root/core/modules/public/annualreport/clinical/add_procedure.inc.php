...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["title"]			= "Add Procedure";
	$PAGE_META["site"]			= "Procedure portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/research?section=add_procedure", "title" => "Add Procedure");
	
	// This grid should be expanded upon redirecting back to the clinical index.
	$_SESSION["clinical_expand_grid"] = "procedures_grid";
	
	echo "<h1>Add Procedure</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_procedures", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						application_log("success", "Procedure [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an Procedure record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_URL; ?>/annualreport/clinical?section=add_procedure&amp;step=2" method="post">
			<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Procedure">
			<colgroup>
				<col style="width: 3%" />
				<col style="width: 20%" />
				<col style="width: 77%" />
