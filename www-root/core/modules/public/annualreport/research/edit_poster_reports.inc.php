...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($POSTER_REPORTS_ID) {
		$query	= "SELECT * FROM `ar_poster_reports` WHERE `poster_reports_id`=".$db->qstr($POSTER_REPORTS_ID)." AND `proxy_id` = ".$db->qstr($ENTRADA_USER->getActiveId());
		...
		if($result) {
			...
			echo "<h1>Edit Poster Presentations / Technical Reports</h1>\n";

			// Error Checking
			switch($STEP) {
				case 2 :					
					/**
					 * Non-Required field "pubmed_id" / PubMed ID.
					 */
					...
					/**
					 * Required field "source" / Source / Presented @			 
					 */
					...
					/**
					 * Non-Required field "emonth" / Date E-Published		 
					 */
					...
					/**
					 * Non-Required field "volume" / Volume
					 */
					...
					/**
					 * Non-Required field "pages" / Pages
					 */
					...
					if(!$ERROR) {
						...
						if($db->AutoExecute("ar_poster_reports", $PROCESSED, "UPDATE", "`poster_reports_id`=".$db->qstr($POSTER_REPORTS_ID))) {
								switch($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["post_action"]) {
									case "new" :
										$url = ENTRADA_URL."/annualreport/research?section=add_poster_reports";
										$msg	= "You will now be redirected to add more Poster Presentations / Technical Reports; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.";
									break;
									case "index" :
									default :
								...
								application_log("success", "Edited Poster Presentations / Technical Reports [".$POSTER_REPORTS_ID."] in the system.");					
		
						} else {
							$ERROR++;
							$ERRORSTR[] = "There was a problem editing this Poster Presentations / Technical Reports record in the system. The MEdIT Unit was informed of this error; please try again later.";
		
							application_log("error", "There was an error editing the Poster Presentations / Technical Reports. Database said: ".$db->ErrorMsg());
						}
					} else {
						$STEP = 1;
					}
			...
			switch($STEP) {
				...
				default :
					...
					{
						$bookChapterMonoQuery = "SELECT * FROM `ar_poster_reports` WHERE `poster_reports_id` =".$db->qstr($POSTER_REPORTS_ID);
					...
					?>
					<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=edit_poster_reports&amp;step=2&amp;rid=<?php echo $POSTER_REPORTS_ID;?>"" method="post">
					...
					<tr>
						<td></td>
						<td style="vertical-align: top"><label for="source" class="form-required">Source / Presented @</label></td>
					...
					<tr>
						...
						<option value = ""></option>
						<?php
							if (isset($PROCESSED["epub_date"]) && strlen($PROCESSED["epub_date"]) == 5) {
								$epubmonth 	= substr($PROCESSED["epub_date"], 0, 1);
								$epubyear 	= substr($PROCESSED["epub_date"], 1, 4);
							} else if(isset($PROCESSED["epub_date"]) && strlen($PROCESSED["epub_date"]) == 6) {
								$epubmonth 	= substr($PROCESSED["epub_date"], 0, 2);
								$epubyear 	= substr($PROCESSED["epub_date"], 2, 4);
							} else {
								$epubmonth 	= "";
								$epubyear 	= "";
							}
					...
					<tr>
						<td></td>
						<td style="vertical-align: top"><label for="volume" class="form-nrequired">Volume</label></td>
					...
					<tr>
						<td></td>
						<td style="vertical-align: top"><label for="pages" class="form-nrequired">Pages</label></td>
					...
					<tr>
						...
						<option value = ""></option>
						<?php
							$types = getPublicationTypesSpecific(array("Poster Presentations", "Technical Report"));
			...
			$ERROR++;
			$ERRORSTR[] = "In order to edit a Poster Presentations / Technical Reports record you must provide a valid Poster Presentations / Technical Reports record identifier. The provided ID does not exist in this system.";

			echo display_error();

			application_log("notice", "Failed to provide a valid Poster Presentations / Technical Reports record identifer when attempting to edit a Poster Presentations / Technical Reports record.");
		}
	} else {
		$ERROR++;
		$ERRORSTR[] = "In order to edit a Poster Presentations / Technical Reports record you must provide the Poster Presentations / Technical Reports record identifier.";

		echo display_error();

		application_log("notice", "Failed to provide Poster Presentations / Technical Reports record identifer when attempting to edit a Poster Presentations / Technical Reports record.");
	}
}
?>