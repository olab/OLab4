...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$NON_PEER_REVIEWED_PAPERS_ID = $_GET["rid"];
	...
	if($NON_PEER_REVIEWED_PAPERS_ID) {
		$query	= "SELECT * FROM `ar_non_peer_reviewed_papers` WHERE `non_peer_reviewed_papers_id`=".$db->qstr($NON_PEER_REVIEWED_PAPERS_ID)." AND `proxy_id` = ".$db->qstr($ENTRADA_USER->getActiveId());
		...
		if($result) {
			...
			echo "<h1>Edit Non-Peer Reviewed Publication</h1>\n";
			...
			switch($STEP) {
				case 2 :					
					...
					if(!$ERROR) {
						...
						if($db->AutoExecute("ar_non_peer_reviewed_papers", $PROCESSED, "UPDATE", "`non_peer_reviewed_papers_id`=".$db->qstr($NON_PEER_REVIEWED_PAPERS_ID))) {
								switch($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["post_action"]) {
									case "new" :
										$url = ENTRADA_URL."/annualreport/research?section=add_non_peer_reviewed";
										$msg	= "You will now be redirected to add more Non-Peer Reviewed Publication; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.";
									break;
									case "index" :
									default :
								...
								application_log("success", "Edited Non-Peer Reviewed Publication [".$NON_PEER_REVIEWED_PAPERS_ID."] in the system.");					
		
						} else {
							$ERROR++;
							$ERRORSTR[] = "There was a problem editing this Non-Peer Reviewed Publication record in the system. The MEdIT Unit was informed of this error; please try again later.";
		
							application_log("error", "There was an error editing the Non-Peer Reviewed Publication. Database said: ".$db->ErrorMsg());
						}
					} else {
						$STEP = 1;
					}
			...
			switch($STEP) {
				...
				default :
					...
					{
						$nonPeerReviewedPaperQuery = "SELECT * FROM `ar_non_peer_reviewed_papers` WHERE `non_peer_reviewed_papers_id` =".$db->qstr($NON_PEER_REVIEWED_PAPERS_ID);
						$PROCESSED = $db->GetRow($nonPeerReviewedPaperQuery);
					}
					...
					?>
					<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=edit_non_peer_reviewed&amp;step=2&amp;rid=<?php echo $NON_PEER_REVIEWED_PAPERS_ID;?>"" method="post">
					...
					<tr>
						...
						<option value = ""></option>
						<?php
							$types = getPublicationTypesSpecific("Non-Peer-Reviewed");
			...
			$ERROR++;
			$ERRORSTR[] = "In order to edit a Peer-Reviewed Paper record you must provide a valid Peer-Reviewed Paper record identifier. The provided ID does not exist in this system.";

			echo display_error();

			application_log("notice", "Failed to provide a valid Non-Peer Reviewed Publication record identifer when attempting to edit a Peer-Reviewed Paper record.");
		}
	} else {
		$ERROR++;
		$ERRORSTR[] = "In order to edit a Peer-Reviewed Paper record you must provide the Peer-Reviewed Paper record identifier.";

		echo display_error();

		application_log("notice", "Failed to provide Peer-Reviewed Paper record identifer when attempting to edit a Peer-Reviewed Paper record.");
	}
}
?>