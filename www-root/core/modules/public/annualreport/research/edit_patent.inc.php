...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($RESEARCH_ID) {
		$query	= "SELECT * FROM `ar_patent_activity` WHERE `patent_activity_id`=".$db->qstr($RESEARCH_ID)." AND `proxy_id` = ".$db->qstr($ENTRADA_USER->getActiveId());
		$result	= $db->GetRow($query);
		if($result) {
			$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/research?section=edit_patent", "title" => "Edit Patent Activity");
			
			echo "<h1>Edit Patent Activity</h1>\n";
			...
			switch($STEP) {
				case 2 :
					...
					if((isset($_POST["patent_activity_type"])) && ($patent_activity_type = clean_input($_POST["patent_activity_type"], array("notags", "trim")))) {
						...
						$ERROR++;
						$ERRORSTR[] = "The <strong>Patent Activity Type</strong> field is required.";
					}
					/**
					...
					if(!$ERROR) {
						...
						if($db->AutoExecute("ar_patent_activity", $PROCESSED, "UPDATE", "`patent_activity_id`=".$db->qstr($RESEARCH_ID))) {
								...
								application_log("success", "Edited Patent Activity [".$RESEARCH_ID."] added to the system.");					
		
						} else {
							$ERROR++;
							$ERRORSTR[] = "There was a problem inserting this Patent Activity into the system. The MEdIT Unit was informed of this error; please try again later.";
			...
			switch($STEP) {
				...
				default :
					...
					{
						$patentQuery = "SELECT * FROM `ar_patent_activity` WHERE `patent_activity_id` =".$db->qstr($RESEARCH_ID);						
						$patentResult = $db->GetRow($patentQuery);
					}
					...
					?>
					<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=edit_patent&amp;step=2&amp;rid=<?php echo $RESEARCH_ID;?>" method="post">
					<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Editing Patent Activity">
					<colgroup>
						<col style="width: 3%" />
						<col style="width: 20%" />
						<col style="width: 77%" />
					...
					<tr>
						<td></td>
						<td><label for="patent_activity_type" class="form-required">Type Of Activity</label></td>
						<td>
							<?php
								...
								foreach($patentTypeArray as $patent_activity_type) {
									echo "<option value=\"".$patent_activity_type["patent_type"]."\"".((($patentResult["patent_activity_type"] == $patent_activity_type["patent_type"]) || ($PROCESSED["patent_activity_type"] == $patent_activity_type["patent_type"])) ? " selected=\"selected\"" : "").">".html_encode($patent_activity_type["patent_type"])."</option>\n";
								}
							?>
						</td>
					</tr>
					...
					<tr>
						...
						<td><input type="text" id="description" name="description" value="<?php echo ((isset($patentResult["description"])) ? html_encode($patentResult["description"]) : html_encode($PROCESSED["description"])); ?>" maxlength="255" style="width: 95%" /></td>						
					</tr>
					<tr>
						<td colspan="3">&nbsp;</td>
					</tr>
