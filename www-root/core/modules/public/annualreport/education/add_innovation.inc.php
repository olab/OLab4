...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["title"]			= "Add Innovation in Education";
	$PAGE_META["course_name"]	= "Innovation in Education portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	
	// This grid should be expanded upon redirecting back to the education index.
	$_SESSION["education_expand_grid"] = "innovation_grid";
	
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/education?".replace_query(array("section" => "add_innovation")), "title" => "Add Innovation in Education");
	
	echo "<h1>Add Innovation in Education</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_innovation", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						application_log("success", "Innovation [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an innovation record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_URL; ?>/annualreport/education?section=add_innovation&amp;step=2" method="post">
			<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Innovation in Education">
			<colgroup>
				<col style="width: 3%" />
				<col style="width: 20%" />
				<col style="width: 77%" />
			...
			<tr>
				...
				<?php
					...
					foreach($innovationArray as $innovationListValue) {
						echo "<option value=\"".$innovationListValue["innovation_type"]."\"".(($PROCESSED["type"] == $innovationListValue["innovation_type"]) ? " selected=\"selected\"" : "").">".html_encode($innovationListValue["innovation_type"])."</option>\n";
					}
				?>
				</select>
				</td>
