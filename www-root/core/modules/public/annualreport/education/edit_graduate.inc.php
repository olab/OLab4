...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($GRADUATE_TEACHING_ID) {
		...
		if($result) {
			...
			echo "<h1>Edit Graduate Teaching</h1>\n";
				...
				case 2 :
					...
					if((isset($_POST["lec_enrollment"])) && ($lec_enrollment = clean_input($_POST["lec_enrollment"], array("int")))) {
						...
						else if(trim($_POST["lec_enrollment"]) == '')
						{
							...
							$ERROR++;
							$ERRORSTR[] = "The <strong>Lecture Enrollment</strong> field is required.";
						}
					}
					/**
					...
					if(!$ERROR) {
						...
						if($db->AutoExecute("ar_graduate_teaching", $PROCESSED, "UPDATE", "`graduate_teaching_id`=".$db->qstr($GRADUATE_TEACHING_ID))) {
								...
								application_log("success", "Edited Graduate Teaching [".$GRADUATE_TEACHING_ID."] in the system.");					
			...
			switch($STEP) {
				...
				default :
					...
					{
						$graduateTeachingQuery = "SELECT * FROM `ar_graduate_teaching` WHERE `graduate_teaching_id` = ".$db->qstr($GRADUATE_TEACHING_ID);
						$graduateTeachingResult = $db->GetRow($graduateTeachingQuery);
					}
					...
					?>
					<form action="<?php echo ENTRADA_URL; ?>/annualreport/education?section=edit_graduate&amp;step=2&amp;rid=<?php echo $GRADUATE_TEACHING_ID;?>" method="post">
					<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Editing Education">
						...
						<tr>
							...
							<td><input type="text" id="course_number" name="course_number" value="<?php echo ((isset($graduateTeachingResult["course_number"])) ? html_encode($graduateTeachingResult["course_number"]) : html_encode($PROCESSED["course_number"])); ?>" maxlength="255" style="width: 20%" /></td>
						</tr>						
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="course_name" class="form-required">Course Name</label></td>
							<td><input type="text" id="course_name" name="course_name" value="<?php echo ((isset($graduateTeachingResult["course_name"])) ? html_encode($graduateTeachingResult["course_name"]) : html_encode($PROCESSED["course_name"])); ?>" maxlength="255" style="width: 95%" /></td>
						</tr>									
						<tr>
							<td></td>
							...
							<option value=""></option>
							<?php
								if(isset($PROCESSED["assigned"]) || isset($graduateTeachingResult["assigned"]))
								{
									if($PROCESSED["assigned"] == "Yes" || $graduateTeachingResult["assigned"] == "Yes")
									{
						...
						<tr>
							...
							<td><input type="text" id="lec_enrollment" name="lec_enrollment" value="<?php echo ((isset($graduateTeachingResult["lec_enrollment"])) ? html_encode($graduateTeachingResult["lec_enrollment"]) : html_encode($PROCESSED["lec_enrollment"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="lec_hours" class="form-required">Lecture Hours</label></td>
							<td><input type="text" id="lec_hours" name="lec_hours" value="<?php echo ((isset($graduateTeachingResult["lec_hours"])) ? html_encode($graduateTeachingResult["lec_hours"]) : html_encode($PROCESSED["lec_hours"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
							<td><label for="lab_enrollment" class="form-required">Lab Enrollment</label></td>
							<td><input type="text" id="lab_enrollment" name="lab_enrollment" value="<?php echo ((isset($graduateTeachingResult["lab_enrollment"])) ? html_encode($graduateTeachingResult["lab_enrollment"]) : html_encode($PROCESSED["lab_enrollment"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="lab_hours" class="form-required">Lab Hours</label></td>
							<td><input type="text" id="lab_hours" name="lab_hours" value="<?php echo ((isset($graduateTeachingResult["lab_hours"])) ? html_encode($graduateTeachingResult["lab_hours"]) : html_encode($PROCESSED["lab_hours"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
							<td><label for="tut_enrollment" class="form-required">Tutorial Enrollment</label></td>							
							<td><input type="text" id="tut_enrollment" name="tut_enrollment" value="<?php echo ((isset($graduateTeachingResult["tut_enrollment"])) ? html_encode($graduateTeachingResult["tut_enrollment"]) : html_encode($PROCESSED["tut_enrollment"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="tut_hours" class="form-required">Tutorial Hours</label></td>							
							<td><input type="text" id="tut_hours" name="tut_hours" value="<?php echo ((isset($graduateTeachingResult["tut_hours"])) ? html_encode($graduateTeachingResult["tut_hours"]) : html_encode($PROCESSED["tut_hours"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>			
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="sem_enrollment" class="form-required">Seminar Enrollment</label></td>							
							<td><input type="text" id="sem_enrollment" name="sem_enrollment" value="<?php echo ((isset($graduateTeachingResult["sem_enrollment"])) ? html_encode($graduateTeachingResult["sem_enrollment"]) : html_encode($PROCESSED["sem_enrollment"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
						...
						<tr>
							...
							<td><input type="text" id="coord_enrollment" name="coord_enrollment" value="<?php echo ((isset($graduateTeachingResult["coord_enrollment"])) ? html_encode($graduateTeachingResult["coord_enrollment"]) : html_encode($PROCESSED["coord_enrollment"])); ?>" maxlength="5" style="width: 40px" /></td>
						</tr>
						<?php } ?>
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="pbl_hours" class="form-required">PBL Hours</label></td>							
							<td><input type="text" id="pbl_hours" name="pbl_hours" value="<?php echo ((isset($graduateTeachingResult["pbl_hours"])) ? html_encode($graduateTeachingResult["pbl_hours"]) : html_encode($PROCESSED["pbl_hours"])); ?>" maxlength="255" style="width: 40px" /></td>
						</tr>
						<tr>
							<td></td>
							<td style="vertical-align: top"><label for="comments" class="form-nrequired">Comments</label></td>
							<td><textarea id="comments" name="comments" style="width: 95%" rows="4"><?php echo ((isset($graduateTeachingResult["comments"])) ? html_encode($graduateTeachingResult["comments"]) : html_encode($PROCESSED["comments"])); ?></textarea></td>
						</tr>
						<tr>
							<td colspan="3">&nbsp;</td>
						</tr>
						<tr>
							...
							if((isset($PROCESSED["year_reported"]) && $PROCESSED["year_reported"] != "")) {
								...
								displayARYearReported($graduateTeachingResult["year_reported"], $AR_CUR_YEAR, $AR_PAST_YEARS, $AR_FUTURE_YEARS, false);
							}
							?>
						</tr>
						<tr>
