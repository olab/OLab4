        ...
        <table class="event-details">
            <tbody>
                ...
                if ($event_contacts) {
                    ...
                    <tr>
                        <th><?php echo $translate->_("Contacts"); ?></th>
                        <td>
                            <ul class="event-contacts"><?php
                                /* $event_contacts is iterated twice, once using sorted keys $contact_keys_sorted and
                                the second time with all remaining keys. */
                                $contact_keys_sorted = array("teacher", "tutor", "ta", "auditor", "week_lead", "other");

                                // Fix for silent failures on server
                                $translate->_options['disableNotices'] = false;

                                foreach ($contact_keys_sorted as $contact_type) {
                                    if (isset($event_contacts[$contact_type])) {
                                        $contacts = $event_contacts[$contact_type];

                                        if ($contact_type == "other") {
                                            $contact_type_title  = "Associated Faculty";
                                        } else {
                                            $contact_type_title =  $translate->_(ucwords(str_replace('_', ' ', $contact_type)));

                                            if (sizeof($contacts) > 1) {
                                                $contact_type_title .= "s";
                                            }
                                        }

                                        ?><li><?php echo $contact_type_title ?></li>
                                        <ul><?php
                                            foreach ($contacts as $contact) {
                                                echo "<li><a href=\"" . ENTRADA_RELATIVE . "/people?id=" . $contact["proxy_id"] . "\" class=\"event-details-item\">" . html_encode($contact["fullname"]) . "</a></li>\n";
                                            }
                                        ?></ul><?php

                                        unset($event_contacts[$contact_type]);
                                    }
                                }

                                // Display remaining contact types. I tried minimizing redundancy by combining everything into one array first but it negatively affected readability.
                                foreach ($event_contacts as $contact_type => $contacts) {
                                    if (in_array($contact_type, $contact_keys_sorted)) {
                                        continue;
                                    }

                                    $contact_type_title = ucwords(str_replace('_', ' ', $contact_type));
...
<div>
    ...
    if ($results && (!$event_info['keywords_hidden'] || $ENTRADA_USER->getActiveGroup() != "student") && isset($event_info['keywords_release_date']) && time() >= $event_info['keywords_release_date']) {
        ...
        ?>
        <a name="event-keywords-section-anchor"></a>
...
<div>
    <?php
    if (defined("EVENT_OBJECTIVES_SHOW_LINKS")) {
        $EVENT_OBJECTIVES_SHOW_LINKS = EVENT_OBJECTIVES_SHOW_LINKS ? true : false;
    } else {
        $EVENT_OBJECTIVES_SHOW_LINKS = false;
    }

    if (!$EVENT_OBJECTIVES_SHOW_LINKS) {
    ...
    if (isset($event_info["objectives_release_date"]) && (($event_info["objectives_release_date"] == 0) || (time() >= $event_info["objectives_release_date"]))) {
        if ($EVENT_OBJECTIVES_SHOW_LINKS) {
            ...
            $event_objectives = $event->getObjectives();
            $objective_repository = Models_Repository_Objectives::getInstance();
            $objectives_have_links = $objective_repository->fetchHasLinks("from", $event_objectives, $version_id, $exclude_tag_set_ids, new Entrada_Curriculum_Context_Specific_Event($event->getID()));
            $event_objective_rows = $objective_repository->populateHasLinks($objective_repository->toArrays($event_objectives), $objectives_have_links);
            $event_objectives_by_tag_set = $objective_repository->groupArraysByTagSet($event_objective_rows);

            $event_objectives_view = new Zend_View();
            $event_objectives_view->setScriptPath(ENTRADA_CORE . "/includes/views/");
            $event_objectives_view->translate = $translate;
            $event_objectives_view->element_id = "event-objectives-section";
            $event_objectives_view->anchor_name = "event-objectives-section";
            $event_objectives_view->section_title = "Event Objectives Section";
            $event_objectives_view->heading_title = "Event Objectives";
            $event_objectives_view->objectives = $event_objectives_by_tag_set;
            $event_objectives_view->event_id = $event->getID();
            $event_objectives_view->cunit_id = $event->getCunitID();
            $event_objectives_view->course_id  = $event->getCourseID();
            $event_objectives_view->cperiod_id  = $event->getCperiodID();
            $event_objectives_view->direction = "from";
            $event_objectives_view->version_id = $version_id;
            $event_objectives_view->exclude_tag_set_ids = $exclude_tag_set_ids;
            echo $event_objectives_view->render("objectives.inc.php");
        } else {
            if ($show_event_objectives || $show_clinical_presentations || $show_curriculum_objectives) {
                $include_objectives = true;

                echo "<a name=\"event-objectives-section-anchor\"></a>\n";
                ...
                if ($show_clinical_presentations) { ?>
                    <div class="section-holder mapped">
                        ...
                        foreach ($clinical_presentations as $key => $result) {
                            $set = Models_Objective::fetchObjectiveSet($result["objective_id"], $ENTRADA_USER->getActiveOrganisation());
                            ?>
                            <li>
            ...
            <div class="section-holder">
                <div id="mapped_objectives" class="mapped">
                    <div id="event-list-wrapper" <?php echo ($explicit_event_objectives)?'':' style="display:none;"';?>>
                        <a name="event-objective-list-anchor"></a>
    ...
    if ($event_resource_entities) {
                        ...
                        switch ($entity->getEntityType()) {
                            ...
                            case 11 :
                                ...
                                if (((int) $resource["release_date"]) && ($resource["release_date"] > time())) {
                                    $resource["description"] .=  "<p class=\"muted resource-description\">This file will be available for downloading <strong>".date(DEFAULT_DATETIME_FORMAT, $resource["release_date"])."</strong>.</p>";
                                } elseif (((int) $resource["release_until"]) && ($resource["release_until"] < time())) {
                                    $resource["description"] .= "<p class=\"muted resource-description\">This file was only available for download until <strong>".date(DEFAULT_DATETIME_FORMAT, $resource["release_until"])."</strong>. Please contact the primary teacher for assistance if required.</p>";
                                }
                            ...
                            case 3 :
                                ...
                                if (((int) $resource["release_date"]) && ($resource["release_date"] > time())) {
                                    $resource["description"] .=  "<p class=\"muted resource-description\">This link will become accessible <strong>".date(DEFAULT_DATETIME_FORMAT, $resource["release_date"])."</strong>.</p>";
                                } elseif (((int) $resource["release_until"]) && ($resource["release_until"] < time())) {
                                    $resource["description"] .= "<p class=\"muted resource-description\">This link was only accessible until <strong>".date(DEFAULT_DATETIME_FORMAT, $resource["release_until"])."</strong>. Please contact the primary teacher for assistance if required.</p>";
                                }
                            ...
                            case 7 :
                                ...
                                if (((int) $resource["release_date"]) && ($resource["release_date"] > time())) {
                                    $resource["description"] .=  "<p class=\"muted resource-description\">This learning module will become accessible <strong>".date(DEFAULT_DATETIME_FORMAT, $resource["release_date"])."</strong>.</p>";
                                } elseif (((int) $resource["release_until"]) && ($resource["release_until"] < time())) {
                                    $resource["description"] .= "<p class=\"muted resource-description\">This learning module was only accessible until <strong>".date(DEFAULT_DATETIME_FORMAT, $resource["release_until"])."</strong>. Please contact the primary teacher for assistance if required.</p>";
                                }
                            ...
                            case 8 :
                                ...
                                if ((int) $quiz->getReleaseDate() && (int) $quiz->getReleaseUntil()) {
                                    $resource["description"] .= "<p class=\"muted resource-description\">This quiz " . ($quiz->getReleaseUntil() > time() ? "is" : "was only") .  " available from <strong>".date(DEFAULT_DATETIME_FORMAT, html_encode($quiz->getReleaseDate()))."</strong> to <strong>".date(DEFAULT_DATETIME_FORMAT, html_encode($quiz->getReleaseUntil()))."</strong>.</p>";
                                } elseif ((int) $quiz->getReleaseDate()) {
                                    if ($quiz->getReleaseDate() > time()) {
                                        $resource["description"] .= "<p class=\"muted resource-description\">You will be able to attempt this quiz starting <strong>".date(DEFAULT_DATETIME_FORMAT, html_encode($quiz->getReleaseDate()))."</strong>.</p>";
                                    } else {
                                        $resource["description"] .= "<p class=\"muted resource-description\">This quiz has been available since <strong>".date(DEFAULT_DATETIME_FORMAT, html_encode($quiz->getReleaseDate()))."</strong>.</p>";
                                    }
                                } elseif ((int) $quiz->getReleaseUntil()) {
                                    if ($quiz->getReleaseUntil() > time()) {
                                        $resource["description"] .= "<p class=\"muted resource-description\">You will be able to attempt this quiz until <strong>".date(DEFAULT_DATETIME_FORMAT, html_encode($quiz->getReleaseUntil()))."</strong>.</p>";
                                    } else {
                                        $resource["description"] .= "<p class=\"muted resource-description\">This quiz was only available until <strong>".date(DEFAULT_DATETIME_FORMAT, html_encode($quiz->getReleaseUntil()))."</strong>. Please contact a teacher for assistance if required.</p>";
                                    }
                                } else {
                                        ...
                                        switch ($entry->getProgressValue()) {
                                            case "complete" :
                                                if (($quiz_type_code == "delayed" && $quiz->getReleaseUntil() <= time()) || ($quiz_type_code == "immediate")) {
                                                    ...
                                                    $resource["description"] .=	date(DEFAULT_DATETIME_FORMAT, $entry->getUpdatedDate())." <strong>Score:</strong> ".$entry->getQuizScore()."/".$entry->getQuizValue()." (".$percentage."%)";
                                                    ...
                                                    $resource["description"] .= "<li>".date(DEFAULT_DATETIME_FORMAT, $entry->getUpdatedDate())." - <strong>Completed</strong></li>";
                                                } else {
                                                    $resource["description"] .= "<li>".date(DEFAULT_DATETIME_FORMAT, $entry->getUpdatedDate())." <strong>Score:</strong> To Be Released ".date(DEFAULT_DATETIME_FORMAT, $quiz->getReleaseUntil())."</li>";
                                                }
                                                break;
                                            case "expired" :
                                                $resource["description"] .= "<li class=\"incorrect\">".date(DEFAULT_DATETIME_FORMAT, $entry->getUpdatedDate())." <strong>Expired Attempt</strong>: not completed.</li>";
                                                break;
                                            case "inprogress" :
                                                $resource["description"] .= "<li>".date(DEFAULT_DATETIME_FORMAT, $entry->getUpdatedDate())." <strong>Attempt In Progress</strong> ( <a href=\"".ENTRADA_RELATIVE."/quizzes?section=attempt&amp;id=".html_encode($quiz->getAquizID())."\">continue quiz</a> )</li>";
                                                break;
                                            default :
                                                continue;
                                                break;
                            ...
                            case 12 :
                                ...
                                break;
                            case 13 :
                                $form = Models_Assessments_Form::fetchRowByID($resource["form_id"]);
                                if ($form) {
                                    if ((((!(int) $resource["release_date"]) || ($resource["release_date"] <= time())) && ((!(int) $resource["release_until"]) || ($resource["release_until"] >= time())))) {
                                        $resource["title"] = "<a class='event-evaluation-resource-link resource-link' href='#' data-event-id='".$event_info["event_id"]."' data-form-id='".$resource["form_id"]."'>". $form->getTitle() ."</a>\n";
                                    } else {
                                        $resource["title"] = "<p class=\"resource-title\">" . $form->getTitle() . "</p>";
                                    }
    
                                    if (((int) $resource["release_date"]) && ($resource["release_date"] > time())) {
                                        $resource["description"] =  "<p class=\"muted resource-description\">" . $translate->_("This learning module will become accessible") . " <strong>".date(DEFAULT_DATE_FORMAT, $resource["release_date"])."</strong>.</p>";
                                    } elseif (((int) $resource["release_until"]) && ($resource["release_until"] < time())) {
                                        $resource["description"] = "<p class=\"muted resource-description\">". $translate->_("This learning module was only accessible until") . " <strong>".date(DEFAULT_DATE_FORMAT, $resource["release_until"])."</strong>. " . $translate->_("Please contact the primary teacher for assistance if required.") . "</p>";
                                    } else {
                                        $resource["description"] = "<p class=\"muted resource-description\">" . $form->getDescription() . "</p>";
                                    }

                                    $resource["type"] = $translate->_("FeedBack Form");
                                    $resource["type_id"] = $entity->getEntityType();
                                }
                                break;
                        }
        ...
        if ($display_pre || $display_during || $display_post || $display_none) {
            ?>
            <a name="event-resources-section-anchor"></a>
            <h2 class="no-printing" title="Event Resources Section">Event Resources</h2>
            <div class="no-printing" id="event-resources-section">
                    ...
                    if ($entity_timeframe_pre) { ?>
                        <div id="event-resource-timeframe-pre-container" class="resource-list">
                            <div class="resource-container-pre">
                                <h2 class="timeframe-heading">Before Class</h2>
                    ...
                    if ($entity_timeframe_during) { ?>
                        <div id="event-resource-timeframe-during-container" class="resource-list">
                            <div class="resource-container-during">
                                <h2 class="timeframe-heading">During Class</h2>
                    ...
                    if ($entity_timeframe_post) { ?>
                        <div id="event-resource-timeframe-post-container" class="resource-list">
                            <div class="resource-container-post">
                                <h2 class="timeframe-heading">After Class</h2>
                    ...
                    if ($entity_timeframe_none) { ?>
                        <div id="event-resource-timeframe-none-container" class="resource-list">
                            <div class="resource-container-none">
                                <h2 class="timeframe-heading">No Timeframe</h2>
