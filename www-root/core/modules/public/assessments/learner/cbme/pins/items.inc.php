...
if(!defined("PARENT_INCLUDED")) {
    ...
    if (!$ERROR) {
        ...
        $cbme_progress_api = new Entrada_CBME_Visualization(array(
            ...
            "filters" => $_GET,
            "query_limit" => 12,
            "limit_dataset" => array("item_pins", "pinned_item_count", "filtered_pinned_item_count", "course_epas", "roles", "course_milestones", "course_assessment_tools", "rating_scales", "filter_list_data", "course_stages", "unread_assessment_count"),
            "courses" => $courses,
        ...
        if ($learner) {
             ...
             */
            $cbme_items_view = new Views_CBME_Items();
            $cbme_items_view->render(array(
                "items" => $dataset["item_pins"],
                "total_count" => $dataset["pinned_item_count"],
                "filtered_count" => $dataset["filtered_pinned_item_count"],
                ...
                "form_action_url" => ENTRADA_URL . "/assessments/learner/cbme/pins/items?proxy_id=" . html_encode($PROCESSED["proxy_id"] ? $PROCESSED["proxy_id"] : $ENTRADA_USER->getActiveId()),
                "form_reset_url" => ENTRADA_URL . "/assessments/learner/cbme/pins/items?proxy_id=" . html_encode($PROCESSED["proxy_id"] ? $PROCESSED["proxy_id"] : $ENTRADA_USER->getActiveId()),
