...
if(!defined("PARENT_INCLUDED")) {
    ...
    if (!$ERROR) {
        ...
        $cbme_progress_api = new Entrada_CBME_Visualization(array(
            ...
            "filters" => $_GET,
            "query_limit" => 12,
            "limit_dataset" => array("assessment_pins", "items", "total_pinned_assessment_count", "filtered_pinned_assessment_count", "pinned_item_count", "course_epas", "roles", "course_milestones", "course_assessment_tools", "rating_scales", "filter_list_data", "course_stages", "unread_assessment_count"),
            "courses" => $courses,
        ...
        if ($learner) {
            ...
            $cbme_assessments_view->render(array(
                "assessments" => $dataset["assessment_pins"],
                "total_count" => $dataset["total_pinned_assessment_count"],
                "filtered_count" => $dataset["filtered_pinned_assessment_count"],
                ...
                "form_action_url" => ENTRADA_URL . "/assessments/learner/cbme/pins/assessments?proxy_id=" . html_encode($PROCESSED["proxy_id"]),
                "form_reset_url" => ENTRADA_URL . "/assessments/learner/cbme/pins/assessments?proxy_id=" . html_encode($PROCESSED["proxy_id"]),
