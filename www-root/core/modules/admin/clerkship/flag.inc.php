 ...
 *
 * Allows faculty/staff to flag an entry in the system if they have the appropriate access.
 *
...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CLERKSHIP"))) {
	...
	if (isset($_POST["entry_id"]) && (clean_input($_POST["entry_id"], "int"))) {
		$RECORD_ID = clean_input($_POST["entry_id"], "int");
	}
	if ($RECORD_ID) {
		...
		if ($PROCESSED) {
			...
			switch($STEP) {
				case 2 :
					...
					if ($db->Execute($query)) {
						$url = ENTRADA_URL."/admin/clerkship/clerk?ids=".$PROCESSED["proxy_id"];
							...
							<td colspan="3" style="padding-top: 25px">
								...
								<tr>
									<td style="width: 25%; text-align: left">
										<input type="button" class="btn" value="Cancel" onclick="window.location='<?php echo ENTRADA_URL; ?>/admin/clerkship/clerk?ids=<?php echo $PROCESSED["proxy_id"]; ?>'" />
									</td>
									<td style="width: 75%; text-align: right; vertical-align: middle">
										<form action="<?php echo ENTRADA_URL ?>/admin/clerkship/flag?<?php echo replace_query(array ("step" => 2)); ?>" method="POST">
											<input type="hidden" value="<?php echo $RECORD_ID; ?>" name="entry_id" />
											<input type="submit" class="btn btn-primary" value="Confirm Flag" />
										</form>
									</td>
								</tr>
								</table>
