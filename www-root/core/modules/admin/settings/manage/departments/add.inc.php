 ...
 *
 * This file adds a department to the `departments` table.
 *
...
if (!defined("PARENT_INCLUDED") || !defined("IN_CONFIGURATION")) {
	...
	$BREADCRUMB[] = array("url" => ENTRADA_URL . "/admin/settings/manage/departments?" . replace_query(array("section" => "add")) . "&amp;org=" . $ORGANISATION_ID, "title" => "Add");
	...
	switch ($STEP) {
		case 2 :
			...
			if (!$ERROR) {
				...
				if ($db->AutoExecute(AUTH_DATABASE . ".departments", $PROCESSED, "INSERT")) {
					if ($DEPARTMENT_ID = $db->Insert_Id()) {
						$params = array("department_id" => $DEPARTMENT_ID, "organisation_id" => $ORGANISATION_ID);
						$url = ENTRADA_URL . "/admin/settings/manage/departments?org=" . $ORGANISATION_ID;
						$SUCCESS++;
						$SUCCESSSTR[] = "You have successfully added <strong>" . html_encode($PROCESSED["department_title"]) . "</strong> to the system.<br /><br />You will now be redirected to the Departments index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"" . $url . "\" style=\"font-weight: bold\">click here</a> to continue.";
						$ONLOAD[] = "setTimeout('window.location=\\'" . $url . "\\'', 5000)";
						application_log("success", "New Department [" . $DEPARTMENT_ID . "] added to the system.");
					} else {
						$ERROR++;
						$ERRORSTR[] = "There was a problem inserting this Department into the system. The system administrator was informed of this error; please try again later.";
...
?>
			<form action="<?php echo ENTRADA_URL . "/admin/settings/manage/departments" . "?" . replace_query(array("action" => "add", "step" => 2)) . "&org=" . $ORGANISATION_ID; ?>" id="department_add_form" method="post">
				<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Page">
					...
					<thead>
						<tr>
							<td colspan="2"><h1>Add Department</h1></td>
						</tr>
					</thead>
					<tfoot>
						<tr>
					...
					<tbody>
						...
						?>
								<option value="<?php echo $result["entity_id"] ?>">
