    ...
    if ($descriptors) {
        ...
        foreach ($descriptors as $descriptor) {
            if (!isset($search_value) || stripos(($descriptor->getDescriptor()), $search_value) !== false) {
                $url = ENTRADA_URL . "/admin/settings/manage/descriptors?section=edit&id=".$descriptor->getID()."&org=".$ORGANISATION_ID;
                if ($count >= $start && $count < ($start + $limit)) {
                    $row = array();
                    $row["checkbox"] = "<input class=\"delete\" type=\"checkbox\" name=\"delete[".html_encode($descriptor->getID())."]\" value=\"".html_encode($descriptor->getID())."\" />";
