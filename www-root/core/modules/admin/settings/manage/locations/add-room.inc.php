 ...
 *
 * This file adds a room to the `global_lu_rooms` table.
 *
...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CONFIGURATION"))) {
    ...
    if (isset($_GET["building_id"]) && ($building_id = clean_input($_GET["building_id"], "int")) && $building_id != 0) {
    ...
    $BREADCRUMB[]  = array("url" => ENTRADA_URL . "/admin/settings/manage/locations?" . replace_query(array("section" => "add-room")) . "&amp;org=" . $ORGANISATION_ID, "title" => "Add Room");
    ...
    switch ($STEP) {
        case 2 :
            ...
            if (isset($_POST["room_number"]) && ($room_number = clean_input($_POST["room_number"], array("notags", "trim"))) && strlen($room_number) <= 20) {
                ...
                $room = Models_Location_Room::fetchRowByNumber($room_number, $building->getID());
                if ($room && is_object($room)) {
                    add_error("The Room Number you selected already exists, please choose another.");
                }
            } else {
            ...
            if (!$ERROR) {
                $room = new Models_Location_Room($PROCESSED);
                if ($room->insert()) {
                    if ($ROOM_ID = $room->getID()) {
                        ...
                        $success_message = $BUILDING_TEXT["success_msg"]["success_add_1"] . "<strong>" . html_encode($PROCESSED["room_number"]) . "</strong>" . $BUILDING_TEXT["success_msg"]["success_add_2"];
                        ...
                        application_log("success", "New Room [" . $PROCESSED["room_number"] . "] added to the system.");
                    } else {
    ...
    ?>
    <form class="form-horizontal" action="<?php echo ENTRADA_URL . "/admin/settings/manage/locations" . "?" . replace_query(array("step" => 2)) . "&org=" . $ORGANISATION_ID; ?>" id="room_add_form" method="post">

        <h1>
            <?php echo $BUILDING_TEXT["add_room"];?>
        </h1>
