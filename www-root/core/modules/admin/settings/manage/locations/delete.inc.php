 ...
 *
 * This file deletes a building from the `global_lu_buildings` table.
 *
...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CONFIGURATION"))) {
    ...
    $BUILDING_TEXT = $MODULE_TEXT["building"]["building"]["delete"];
    ?>
    <h1>
        <?php echo $BUILDING_TEXT["title"];?>
    </h1>
    <?php
    $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/settings/manage/locations?section=delete&amp;org=".$ORGANISATION['organisation_id'], "title" => "Delete Buildings");
    ...
    if ($PROCESSED["remove_ids"]) {
        switch ($STEP) {
            case 2:
                foreach ($PROCESSED["remove_ids"] as $id) {
                    $query = "DELETE FROM `global_lu_buildings`
                              WHERE `building_id` = ".$db->qstr($id);
                    if (!$db->Execute($query)) {
                        add_error("Error deleting building.");
                    }
                ...
                $ONLOAD[] = "setTimeout('window.location=\\'".ENTRADA_URL."/admin/settings/manage/locations/?org=".$ORGANISATION_ID."\\'', 5000)";
                break;
            case 1:
            default:
            ...
            <form action ="<?php echo ENTRADA_URL."/admin/settings/manage/locations/?section=delete&org=".$ORGANISATION_ID."&step=2";?>" method="post">
                <table class="tableList" cellspacing="0" summary="<?php echo $BUILDING_TEXT["label_list"];?>">
                    ...
                    <thead>
                        <tr>
                            ...
                            <td class="title"><?php echo $BUILDING_TEXT["label_building"];?></td>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        foreach ($PROCESSED["remove_ids"] as $id) {
                            $building = Models_Location_Building::fetchRowByID($id);
                            if ($building && is_object($building)) {
                                ?>
                                <tr>
                                    <td><input id="building_<?php echo $id;?>" type="checkbox" value="<?php echo $id;?>" name="remove_ids[]" checked="checked"/></td>
                                    <td><label for="building_<?php echo $id;?>"><?php echo $building->getBuildingName();?></label></td>
                                </tr>
                                <?php
                            }
                        } ?>
        ...
        $error_message .= "<br /><br />" . $BUILDING_TEXT["error_msg"]["error_add_1"] . "<strong>" . $BUILDING_TEXT["error_msg"]["error_add_2"] . "</strong>" . $BUILDING_TEXT["error_add_3"]["success_add_3"];
