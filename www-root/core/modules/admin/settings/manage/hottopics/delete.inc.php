...
<?php
$BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/settings/manage/hottopics?section=delete&amp;org=".$ORGANISATION['organisation_id'], "title" => "Delete Hot Topics");
...
if ($PROCESSED["remove_ids"]) {
	switch($STEP){
		...
		foreach ($PROCESSED["remove_ids"] as $id){
			...
			if($num_uses > 1){
				...
				if($db->Execute($query)){
					$SUCCESS++;
					$SUCCESSSTR[] = "Successfully removed Hot Topic [".$id."] from your the system.<br />You will now be redirected to the Hot Topics index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".ENTRADA_URL."/admin/settings/manage/hottopics/?org=".$ORGANISATION_ID."\" style=\"font-weight: bold\">click here</a> to continue.";
				}
				else{
					$ERROR++;
		...
		default:			
			...
			<form action ="<?php echo ENTRADA_URL."/admin/settings/manage/hottopics/?section=delete&org=".$ORGANISATION_ID."&step=2";?>" method="post">
					<table class="tableList" cellspacing="0" summary="List of Curriculum Layout">
						...
						<tbody>
							<?php 
								foreach ($PROCESSED["remove_ids"] as $id) {
									$query = "SELECT * FROM `events_lu_topics` WHERE `topic_id` = ".$db->qstr($id);
	...
	$ERROR++;
	$ERRORSTR[] = "No Hot Topics were selected to be deleted. You will now be redirected to the Hot Topics index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".ENTRADA_URL."/admin/settings/manage/hottopics/?org=".$ORGANISATION_ID."\" style=\"font-weight: bold\">click here</a> to continue.";
