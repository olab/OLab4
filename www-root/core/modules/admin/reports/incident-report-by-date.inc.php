...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_REPORTS"))) {
	...
	$BREADCRUMB[]	= array("url" => "", "title" => "Open Incidents By Incident Date" );
	...
	$query	= "
			...
			ON a.`proxy_id` = b.`id`
			WHERE a.`incident_date` BETWEEN ".$db->qstr($_SESSION[APPLICATION_IDENTIFIER][$MODULE]["reporting_start"])." AND ".$db->qstr($_SESSION[APPLICATION_IDENTIFIER][$MODULE]["reporting_finish"]).$organisation_where."
	...
	<thead>
		<tr>
			...
			<td class="date sortedASC"><a>Incident Date</a></td>
