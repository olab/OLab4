...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_AAMC_CI"))) {
	...
	$BREADCRUMB[] = array("url" => ENTRADA_RELATIVE."/admin/reports/aamc/manage?".replace_query(array("id" => $REPORT_ID, "section" => "edit")), "title" => "Edit Report");

	$PROCESSED["organisation_id"] = (int) $REPORT["organisation_id"];
	...
	echo "<h1>Edit AAMC Curriculum Inventory Report</h1>\n";

	// Error Checking
	switch($STEP) {
		case 2 :
            $PROCESSED["report_language"] = $REPORT["report_language"];
            $PROCESSED["report_active"] = $REPORT["report_active"];
            $PROCESSED["report_status"] = $REPORT["report_status"];
			...
			if ((isset($_POST["report_title"])) && ($report_title = clean_input($_POST["report_title"], array("notags", "trim")))) {
				$query = "SELECT * FROM `reports_aamc_ci` WHERE `report_title` LIKE ".$db->qstr($report_title)." AND `report_active` = 1 AND `raci_id` <> ".$db->qstr($REPORT_ID);
			...
			if (!$ERROR) {
				...
				if ($db->AutoExecute("reports_aamc_ci", $PROCESSED, "UPDATE", "raci_id = ".$db->qstr($REPORT_ID))) {
                    $url = ENTRADA_URL . "/admin/reports/aamc/manage?id=".$REPORT_ID."&action=edit";

                    add_success("You have successfully updated <strong>".html_encode($PROCESSED["report_title"])."</strong>.<br /><br />You will now be redirected back to the report; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.");

                    $ONLOAD[] = "setTimeout('window.location=\\'".$url."\\'', 5000)";

                    application_log("success", "Updated AAMC curriculum inventory report [".$REPORT_ID."].");
				} else {
		...
		default :
            ...
            $PROCESSED["report_year"] = date("Y", strtotime($PROCESSED["report_start"]));

            if (isset($REPORT["report_params"]) && $REPORT["report_params"]) {
                $report_params = json_decode($REPORT["report_params"], true);

                if ($report_params && is_array($report_params)) {

                    $PROCESSED["academic_levels"] = array();
                    $PROCESSED["academic_level_proxy_ids"] = array();

                    foreach ($report_params as $academic_level => $proxy_ids) {
                        $PROCESSED["academic_levels"][] = $academic_level;
                        $PROCESSED["academic_level_proxy_ids"][$academic_level] = implode(",", $proxy_ids);
                    }
                }
            }
		break;
	...
	switch($STEP) {
		...
		default :
			display_status_messages();
			?>
			<form action="<?php echo ENTRADA_RELATIVE; ?>/admin/reports/aamc/manage?id=<?php echo $REPORT_ID; ?>&amp;section=edit&amp;step=2" method="post" id="editAAMCCiReport" class="form-horizontal">
                        ...
                        if ($academic_levels) {
                            foreach ($academic_levels as $level) {
                                ...
                                <div class="academic-level-container">
                                    <label class="checkbox"><input type="checkbox" name="academic_levels[]" id="academic-level-<?php echo $level["curriculum_type_id"]; ?>" value="<?php echo $level["curriculum_type_id"]; ?>"<?php echo ((!isset($PROCESSED["academic_levels"]) || in_array($level["curriculum_type_id"], $PROCESSED["academic_levels"])) ? " checked=\"checked\"" : ""); ?> onclick="jQuery('#academic-level-<?php echo $level["curriculum_type_id"]; ?>-audience').toggle('slow');" /> <?php echo html_encode($level["curriculum_type_name"]); ?></label>
                ...
                <div class="control-group">
                    <a class="btn" href="<?php echo ENTRADA_RELATIVE; ?>/admin/reports/aamc/manage?id=<?php echo $REPORT_ID; ?>">Cancel</a>
