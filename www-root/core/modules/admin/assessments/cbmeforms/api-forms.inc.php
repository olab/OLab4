            ...
            switch ($request["method"]) {
                ...
                case "copy-form" :
                    ...
                    if (!$ERROR) {
                        ...
                        $url = ENTRADA_URL."/admin/assessments/cbmeforms?section=edit-form&cbme_form_id=". $new_form_id;
                ...
                case "update-form-primitives":
                    ...
                    if (isset($request["course_id"]) && $tmp_input = clean_input($request["course_id"], array("trim", "int"))) {
                        $PROCESSED["attributes"]["course_id"] = $tmp_input;
                    }
                    ...
                    $PROCESSED["attributes"] = count($PROCESSED["attributes"]) ? $PROCESSED["attributes"] : null;
                ...
                */
                ...
                break;
                case "save_form":
                    if (isset($_POST["cbme_form_id"]) && $tmp_input = clean_input($_POST["cbme_form_id"], array("trim", "int"))) {
                        $PROCESSED["cbme_form_id"] = $tmp_input;
                    } else {
                        echo json_encode(array("status" => "error", "data" => array($translate->_("A form title is required."))));
                        exit();
                    }
                    ...
                    if (!$form_in_use) {
                        ...
                        if (isset($_POST["original_course_id"]) && $tmp_input = clean_input($_POST["original_course_id"], array("trim", "int"))) {
                            $PROCESSED["original_course_id"] = intval($tmp_input);
                        } else {
                            $PROCESSED["original_course_id"] = 0;
                        }
                        ...
                        // Check for contextual variables
                        if (($PROCESSED["original_course_id"] == $PROCESSED["course_id"]) &&
                                isset($_POST["contextual_vars_0_1"]) && is_array($_POST["contextual_vars_0_1"])) {
                            $contextual_vars = array_map(function ($cvar) {
                                return clean_input($cvar, array("trim", "int"));
                            }, $_POST["contextual_vars_0_1"]);
                        ...
                        if (!$ERROR) {
                            $PROCESSED["attributes"] = array(
                                ...
                                "contextual_vars" => $PROCESSED["contextual_vars"],
                                "entrustment_rating" => $PROCESSED["entrustment_rating"],
                                "feedback" => $PROCESSED["feedback"],
                                "concerns" => $PROCESSED["concerns"]
                            );

                            if (!$forms_api->saveCBMEForm($PROCESSED)) {
                                echo json_encode(array("error" => "success", "data" => $forms_api->getErrorMessages()));
                                exit();
                            }
                        } else {
                            echo json_encode(array("status" => "error", "data" => $ERRORSTR));
                ...
                case "publish-form":
                    ...
                    $forms_api->setCBMEFormID($PROCESSED["form_id"]);
                    if (!$forms_api->publishCBMEForm()) {
                        $errors = $forms_api->getErrorMessages();
                        echo json_encode(array("status" => "error", "data" => reset($errors)));
                    } else {
                        if (!$form_data = $forms_api->fetchCBMEFormData(false, false)) {
                            $errors = $forms_api->getErrorMessages();
                            echo json_encode(array("status" => "error", "data" => reset($errors)));
                        } else {
                            // Reorder objectives
                            ...
                            if (count($form_data["objectives"])) {
                                $form_objectives = array_map(function($form_objective) {return $form_objective["objective_code"];}, $form_data["objectives"]);
                                ...
                                foreach ($epas as $epa) {
                                    if (in_array($epa["objective_code"], $form_objectives)) {
                                        $objectives[] = $epa;
                                    }
                                }
                            }
            ...
            switch ($request["method"]) {
                ...
                case "get-child-objectives" :
                    ...
                    if ($PROCESSED["course_id"]) {
                        $child_objectives = Models_Objective::fetchAllByParentIDCBMECourseObjective($PROCESSED["parent_id"], $PROCESSED["course_id"], $ENTRADA_USER->getActiveOrganisation());
                    } else {
                ...
                case "get-cvar-responses-count":
                    ...
                    $child_objectives = Models_Objective::fetchAllByParentIDCBMECourseObjective($PROCESSED["cvar_id"], $PROCESSED["course_id"], $ENTRADA_USER->getActiveOrganisation());
                    $responses_count = is_array($child_objectives) ? count($child_objectives) : 0;
