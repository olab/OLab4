...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_ASSESSMENTS"))) {
    ...
    $assessments_base->getAssessmentPreferences("learner-reports");

    if (isset($_SESSION[APPLICATION_IDENTIFIER]["learner-reports"]["evaluation"]["start_date"])) {
        $start_date = $_SESSION[APPLICATION_IDENTIFIER]["learner-reports"]["evaluation"]["start_date"];
    } else {
        $start_date = null;
    }

    if (isset($_SESSION[APPLICATION_IDENTIFIER]["learner-reports"]["evaluation"]["end_date"])) {
        $end_date = $_SESSION[APPLICATION_IDENTIFIER]["learner-reports"]["evaluation"]["end_date"];
    } else {
        $end_date = null;
    }
    ...
    <h1><?php echo $translate->_("Learner Reports"); ?></h1>
    <div id="msgs"></div>
    <form class="form-horizontal" id="evaluation-form">
        ...
        <div class="control-group <?php echo is_null($start_date) && is_null($end_date) ? "hide" : ""; ?> " id="evaluation-search">
            <label class="control-label" for="choose-evaluation-btn"><?php echo $translate->_("Select Learners:"); ?></label>
            <div class="controls">
                <a href="#" id="choose-evaluation-btn" class="btn" type="button"><?php echo $translate->_("Browse Learners "); ?><i class="icon-chevron-down"></i></a>
            </div>
        </div>
        ...
        <input type="hidden" name="current-page" id="current-page" value="learner-reports"/>
    </form>
    <?php
