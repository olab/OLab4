...
if (!defined("PARENT_INCLUDED") || !defined("IN_OBJECTIVES")) {
    ...
    if ($objective_set) {
        ...
        $BREADCRUMB[] = array("url" => ENTRADA_URL. "/admin/curriculum/tags/objectives?set_id=". $objective_set_id, "title" => $objective_set->getTitle());
        $BREADCRUMB[] = array("title" => "Edit Curriculum Tag Set");

        //object set
        $PROCESSED["objective_code"] = $objective_set->getCode();
        $PROCESSED["objective_name"] = $objective_set->getTitle();
        $PROCESSED["objective_shortname"] = $objective_set->getShortname();
        $PROCESSED["objective_description"] = $objective_set->getDescription();
        $PROCESSED["standard"] = $objective_set->getStandard();
        $PROCESSED["languages"] = $objective_set->getLanguages();
        $PROCESSED["maximum_levels"] = $objective_set->getMaximumLevels();
        $PROCESSED["short_method"] = $objective_set->getShortMethod();
        $PROCESSED["long_method"] = $objective_set->getLongMethod();

        //objective
        $objective = Models_Objective::fetchRowBySetIDParentID($objective_set_id, 0);

        $required_fields = json_decode($objective_set->getRequirements(), true);
        if ($required_fields && is_array($required_fields)) {
            foreach ($required_fields as $key => $value) {
                $PROCESSED["requirements_fields"][] = $key;
                if ($value["required"]) {
                    $PROCESSED["required"][] = $key;
                }
            }
        }

        //audience
        $audiences = Models_Objective_Audience::fetchAllByObjectiveIDAudienceType($objective->getID(), "COURSE");
        $course_ids = [];
        if ($audiences) {
            foreach ($audiences as $audience) {
                if ($audience->getAudienceValue() != "all" && $audience->getAudienceValue() != "none") {
                    $PROCESSED["course_ids"][] = $audience->getAudienceValue();
                    $course_ids[] = $audience->getAudienceValue();
                    $PROCESSED["objective_audience"] = "selected";
                } else {
                    $PROCESSED["objective_audience"] = $audience->getAudienceValue();
                }
            }
        } else {
            $PROCESSED["objective_audience"] = "all";
        }
        $old_audience = $PROCESSED["objective_audience"];

        //levels
        $levels = Models_Objective_TagLevel::fetchAllByObjectiveSetID($objective_set_id);
        $current_max_level = $objective_set->getMaximumLevels();
        $PROCESSED["levels"] = [];
        if ($levels) {
            foreach ($levels as $level) {
                $PROCESSED["levels"][] = $level->getLabel();
            }
        }

        //tag attributes
        $attributes = Models_Objective_TagAttribute::fetchAllByObjectiveSetID($objective_set_id);
        $PROCESSED["tagset_ids"] = [];
        if ($attributes) {
            foreach ($attributes as $attribute) {
                $PROCESSED["tagset_ids"][] = $attribute->getTargetObjectiveSetId();
            }
        }
        ...
        switch ($STEP) {
            case 2 :
                ...
                if ($PROCESSED["objective_audience"] == "selected" && empty($PROCESSED["course_ids"])) {
                ...
                if (isset($_POST["tagsets"]) && is_array($_POST["tagsets"])) {
                    foreach ($_POST["tagsets"] as $tag_set) {
                        if ($tmp_input = clean_input($tag_set, array("trim", "int"))) {
                            $tag_sets[] = $tmp_input;
                        }
                    }
                } else {
                ...
                if (!has_error()) {
                    ...
                    if ($objective_set->update()) {
                        if ($old_audience != $PROCESSED["objective_audience"] || (array_diff($PROCESSED["course_ids"], $course_ids) != array_diff($course_ids, $PROCESSED["course_ids"])) || !$audiences) {
                            ...
                            if ($PROCESSED["objective_audience"] == "selected") {
                                foreach ($PROCESSED["course_ids"] as $course_id) {
                                    $objective_audience["audience_value"] = $course_id;
                                    $obj_audience_model = new Models_Objective_Audience($objective_audience);
                                ...
                                $obj_audience_model = new Models_Objective_Audience($objective_audience);
                                if (!$obj_audience_model->insert()) {
                                    add_error($translate->_("There was a problem updating the audience to the system. The system administrator was informed of this error; please try again later."));
                                    application_log("error", "There was an error associating an objective set with objective audience. Database said: " . $db->ErrorMsg());
                                }
                            }
                        }
                        ...
                        if (isset($PROCESSED["levels"]) && !empty($PROCESSED["levels"])) {
                            foreach ($PROCESSED["levels"] as $key => $label) {
                                $level = Models_Objective_TagLevel::fetchRowByObjectiveSetIdLevel($objective_set_id, $key + 1);
                                if ($level) {
                                    if (!$level->fromArray(array("label" => $label))->update()) {
                                        add_error($translate->_("There was a problem updating the tag level to the system. The system administrator was informed of this error; please try again later."));
                                        application_log("error", "There was an error associating an objective set with tag level. Database said: " . $db->ErrorMsg());
                                    }
                                } else {
                                    $tag_level["objective_set_id"] = $objective_set_id;
                                    $tag_level["level"] = $key + 1;
                                    $tag_level["label"] = $label;
                                    $tag_level["updated_date"] = time();
                                    $tag_level["updated_by"] = $ENTRADA_USER->getActiveId();
                                    $level_model = new Models_Objective_TagLevel($tag_level);
                                    if (!$level_model->insert()) {
                                        add_error($translate->_("There was a problem adding the tag level to the system. The system administrator was informed of this error; please try again later."));
                                        application_log("error", "There was an error associating an objective set with tag level. Database said: " . $db->ErrorMsg());
                                    }
                                }
                            }
                        ...
                        if (isset($PROCESSED["maximum_levels"]) && $PROCESSED["maximum_levels"] < $current_max_level) {
                            for ($i = $current_max_level; $i > $PROCESSED["maximum_levels"]; $i--) {
                                $level = Models_Objective_TagLevel::fetchRowByObjectiveSetIdLevel($objective_set_id, $i);
                                if ($level && !$level->delete()) {
                                    add_error($translate->_("There was a problem deleting the tag level to the system. The system administrator was informed of this error; please try again later."));
                                    application_log("error", "There was an error associating an objective set with tag level. Database said: " . $db->ErrorMsg());
                                }
                            }
                        }
                        ...
                        if (!empty($tag_sets)) {
                            ...
                            foreach ($tag_sets as $set_id) {
                                if (!in_array($set_id, $PROCESSED["tagset_ids"])) {
                                    if ($tmp_input = clean_input($set_id, array("trim", "int"))) {
                                        $tag_attribute["target_objective_set_id"] = $set_id;
                                        $tag_attributes_model = new Models_Objective_TagAttribute($tag_attribute);
                        ...
                        $tags_remove_set  = array_diff($PROCESSED["tagset_ids"], $tag_sets);
                        if ($tags_remove_set && is_array($tags_remove_set) && !empty($tags_remove_set)) {
                            foreach ($tags_remove_set as $set) {
                                $tag_attributes_model = Models_Objective_TagAttribute::fetchRowByObjectiveSetIdTargetObjectiveSetID($objective_set_id, $set);
                                if ($tag_attributes_model && is_object($tag_attributes_model)) {
                                    if (!$tag_attributes_model->delete()) {
                                        add_error($translate->_("There was an error when trying to update a tag attribute into the system."));
                                        application_log("error", "Unable to update tag attribute to the database when editing tag set. Database said: " . $db->ErrorMsg());
                                    } else {
                                        if ($target_tags_linked = Models_Objective_LinkedObjective::fetchAllByObjectiveSetIDAsTargetObjectiveID($set)){
                                            foreach ($target_tags_linked as $linked_objective) {
                ...
                if (has_error()) {
                    ...
                    add_success(sprintf($translate->_("You have successfully updated <strong>%s</strong> to the system.<br /><br />You will now be redirected to the objectives index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"%s\" style=\"font-weight: bold\">click here</a> to continue."), html_encode($PROCESSED["objective_name"]) ,$url));

                    $ONLOAD[] = "setTimeout('window.location=\\'" . $url . "\\'', 5000)";

                    application_log("success", "Objective [" . $objective_set_id . "] updated.");
                }
                break;
            case 1 :
            default :
        ...
        switch ($STEP) {
            ...
            default:
                ...
                <h1><?php echo $translate->_("Edit Curriculum Tag Set"); ?></h1>
                <h2 title="Tag Settings"><?php echo $translate->_("Tag Set Details"); ?></h2>
                <form
                    action="<?php echo ENTRADA_URL . "/admin/curriculum/tags?" . replace_query(array("action" => "edit", "step" => 2)); ?>"
                    method="post" class="form-horizontal" id="tags_edit_form">
                    <div class="control-group">
                        <label for="objective_code" class="form-nrequired control-label">Code</label>
                        <div class="controls">
                            <input type="text" id="objective_code" name="objective_code"
                                   value="<?php echo((isset($PROCESSED["objective_code"])) ? html_encode($PROCESSED["objective_code"]) : ""); ?>"
                                   class="span6">
                        </div>
                    </div>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" id="objective_name" name="objective_name"
                                   value="<?php echo((isset($PROCESSED["objective_name"])) ? html_encode($PROCESSED["objective_name"]) : ""); ?>"
                                   class="span10" onkeyup="validateShortname(this.value)">
                        </div>
                    </div>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" id="objective_shortname" name="objective_shortname" disabled
                                   value="<?php echo((isset($PROCESSED["objective_shortname"])) ? html_encode($PROCESSED["objective_shortname"]) : ""); ?>"
                                   class="span6">
                        </div>
                    </div>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea id="objective_description" name="objective_description"
                                      class="span11 expandable"><?php echo((isset($PROCESSED["objective_description"])) ? html_encode($PROCESSED["objective_description"]) : ""); ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="control-group">
                        <div class="controls">
                            <label class="checkbox">
                                <input type="checkbox" id="standard" value="1"
                                       name="standard" <?php echo((isset($PROCESSED["standard"]) && $PROCESSED["standard"] == 1) ? "checked=\"checked\"" : ""); ?> />
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <label class="radio">
                                <input type="radio" name="objective_audience"
                                       value="all" <?php echo(!isset($PROCESSED["objective_audience"]) || (isset($PROCESSED["objective_audience"]) && $PROCESSED["objective_audience"] == "all") ? "checked=\"checked\"" : ""); ?>/> <?php echo $translate->_("all_courses"); ?>
                            </label>

                            <label class="radio">
                                <input type="radio" name="objective_audience"
                                       value="none" <?php echo(isset($PROCESSED["objective_audience"]) && $PROCESSED["objective_audience"] == "none" ? "checked=\"checked\"" : ""); ?>/> <?php echo $translate->_("no_courses"); ?>
                            </label>

                            <div>
                                <label class="radio">
                                    <input type="radio" name="objective_audience"
                                           value="selected" <?php echo(isset($PROCESSED["objective_audience"]) && $PROCESSED["objective_audience"] == "selected" ? "checked=\"checked\"" : ""); ?> /> <?php echo $translate->_("selected_courses"); ?>
                                </label>
                                <button id="choose-course-btn"
                                        class="btn btn-search-filter space-above space-below hide"
                                        style="min-width: 220px; text-align: left;"><?php echo $translate->_("Browse All Courses"); ?>
                                    <i class="icon-chevron-down btn-icon pull-right"></i></button>
                            </div>
                        </div>
                    </div>

                    <h2 title="Tag Options"
                        class="collapsable expanded"><?php echo $translate->_("Tag Options"); ?></h2>
                    <div id="tag-options">
                        <div class="control-group">
                            <?php
                            ...
                            if (isset($language_supported) && !empty($language_supported) && count($language_supported) > 1) { ?>
                                <label
                                    class="form-required control-label"><?php echo $translate->_("Languages"); ?></label>
                                <div class="controls">
                                    <?php foreach ($language_supported as $index => $value) { ?>
                                        <label class="checkbox"><input
                                                type="checkbox" <?php echo(!isset($PROCESSED["languages"]) || (isset($PROCESSED["languages"]) && in_array($index, json_decode($PROCESSED["languages"], true))) ? "checked=\"checked\"" : ""); ?>
                                                name="languages[]" id="lang-"<?php echo $value["name"]; ?>
                            ...
                            <?php } else { ?>
                                <input type="hidden" checked="checked" name="languages[]"
                                       value="<?php echo($language_supported && !empty($language_supported) ? key($language_supported) : "en"); ?>">
                            <?php } ?>
                        </div>
                        <div class="control-group">
                            <label
                                class="form-required control-label no-padding"><?php echo $translate->_("Detail Requirements"); ?></label>
                            <div class="controls">
                                <div class="row-fluid">
                                    <label class="checkbox span3">
                                        <input type="checkbox" name="requirements[]" value="code" <?php echo(!isset($PROCESSED["requirements_fields"]) || (isset($PROCESSED["requirements_fields"]) && in_array("code", $PROCESSED["requirements_fields"])) ? "checked=\"checked\"" : ""); ?>>
                                        <?php echo $translate->_("Code"); ?>
                                    </label>
                                    <label class="checkbox span3 hide" id="req_code">
                                        <input type="checkbox" id="code_checkbox" name="required[]" value="code" <?php echo(isset($PROCESSED["required"]) && in_array("code", $PROCESSED["required"]) ? "checked=\"checked\"" : ""); ?>>
                                        <?php echo $translate->_("Required"); ?>
                                    </label>
                                </div>
                                <div class="row-fluid">
                                    <label class="checkbox span3">
                                        <input type="checkbox" name="requirements[]" value="title" <?php echo(!isset($PROCESSED["requirements_fields"]) || (isset($PROCESSED["requirements_fields"]) && in_array("title", $PROCESSED["requirements_fields"])) ? "checked=\"checked\"" : ""); ?>>
                                        <?php echo $translate->_("Title"); ?>
                                    </label>
                                    <label class="checkbox span3 hide" id="req_title">
                                        <input type="checkbox" id="title_checkbox" name="required[]" value="title" <?php echo(!isset($PROCESSED["required"]) || (isset($PROCESSED["required"]) && in_array("title", $PROCESSED["required"])) ? "checked=\"checked\"" : ""); ?>>
                                        <?php echo $translate->_("Required"); ?>
                                    </label>
                                </div>
                                <div class="row-fluid">
                                    <label class="checkbox span3">
                                        <input type="checkbox" name="requirements[]" value="description" <?php echo(!isset($PROCESSED["requirements_fields"]) || (isset($PROCESSED["requirements_fields"]) && in_array("description", $PROCESSED["requirements_fields"])) ? "checked=\"checked\"" : ""); ?>>
                                        <?php echo $translate->_("Description"); ?>
                                    </label>
                                    <label class="checkbox span3 hide" id="req_description">
                                        <input type="checkbox" id="description_checkbox" name="required[]" value="description" <?php echo(isset($PROCESSED["required"]) && in_array("description", $PROCESSED["required"]) ? "checked=\"checked\"" : ""); ?>>
                                        <?php echo $translate->_("Required"); ?>
                                    </label>
                                </div>
                            </div>
                    ...
                    <h2 title="Tag Display Options"
                        class="collapsable expanded"><?php echo $translate->_("Tag Display Options"); ?></h2>
                    ...
                    <div id="tag-attributes">
                        ...
                        if (Entrada_Settings::fetchValueByShortname("curriculum_tagsets_allow_attributes", $ENTRADA_USER->getActiveOrganisation())) { ?>
                            <div class="control-group">
                                <label for="choose-tagset-btn"
                                       class="control-label"><?php echo $translate->_("Tag Sets"); ?></label>
                                <div class="controls">
                                    <button id="choose-tagset-btn" class="btn btn-search-filter"
                                            style="min-width: 220px; text-align: left;"><?php echo $translate->_("Browse All Tag Sets"); ?>
                                        <i class="icon-chevron-down btn-icon pull-right"></i></button>
                                </div>
                            </div>
                        <?php } ?>
                    ...
                    <div class="control-group">
                        <a href="<?php echo ENTRADA_URL; ?>/admin/curriculum/tags"
                           class="btn"><?php echo $translate->_("global_button_cancel"); ?></a>
                ...
                <script type="text/javascript">
                    jQuery("#choose-course-btn").advancedSearch({
                        ...
                        parent_form: jQuery("#tags_edit_form")
                    });
        ...
        add_error($translate->_("Objective set was not found in the system."));
        echo display_error();
    }
}