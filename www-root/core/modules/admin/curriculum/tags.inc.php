...
if (!defined("PARENT_INCLUDED")) {
    ...
    $URL_ROOT = "/admin";
    $module_title = $translate->_("Curriculum Tag Sets");
    if ($LEARNER_VIEW || !$ENTRADA_ACL->amIAllowed('objectiveattributes', 'update', false)) {
        $URL_ROOT = "";
        $module_title = "Curriculum Tag Sets";
    }
	if (($router) && ($router->initRoute())) {
        ...
        if ( isset($_GET["set_id"]) && $tmp_input = clean_input($_GET["set_id"], array("nows", "int"))) {
            $obj_set = $tmp_input;
        } else {
            $obj_set = 0;
        }

        $BREADCRUMB[] = array("url" => ENTRADA_URL.$URL_ROOT."/curriculum/tags", "title" => $translate->_($module_title));
