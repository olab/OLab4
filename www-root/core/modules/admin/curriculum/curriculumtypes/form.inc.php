...
if ((!defined("PARENT_INCLUDED"))) {
    ...
    if (isset($_GET["cperiod_id"]) && $tmp_input = clean_input($_GET["cperiod_id"], "int")) {
        ...
        if ($ctype) {
            $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/".$MODULE."/".$SUBMODULE."?".replace_query(array("section" => "edit"))."&amp;org=".$ORGANISATION['organisation_id'] ."&amp;type_id=".$ctype->getID(), "title" => $ctype->getCurriculumTypeName());
        }
    }
    ...
    switch ($STEP) {
        case 2 :
            ...
            if ($results instanceof Models_Schedule) {
                ...
                $url = ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=" . $SECTION . "&org=" . $ORGANISATION_ID . (isset($PROCESSED["id"]) ? "&id=" . $PROCESSED["id"] : "") . (isset($PROCESSED["schedule_parent_id"]) ? "&schedule_parent_id=" . $PROCESSED["schedule_parent_id"] : "")."&cperiod_id=".$CPERIOD_ID;
                header("Location: ".$url);
                exit;
            } else {
                if (is_array($results)) {
    ...
    switch ($STEP) {
        ...
        default:
            ...
            if ($breadcrumb_data) {
                ...
                $BREADCRUMB[] = ["url" => ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=block-schedule&org=" . $ORGANISATION["organisation_id"] . "&cperiod_id=" . $schedule->getCperiodID() . "&type_id=" . $ctype->getID(), "title" => "Block Schedule"];
                foreach ($breadcrumb_data as $breadcrumb_element) {
                    $BREADCRUMB[] = array("url" => ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=edit-schedule&org=" . $ORGANISATION['organisation_id'] . "&id=" . $breadcrumb_element["schedule_id"]. "&cperiod_id=" . $schedule->getCperiodID(), "title" => $breadcrumb_element["title"]);
                }
            }
            ...
            <script type="text/javascript">
                jQuery(function($) {
                    ...
                    $("#delete-btn").on("click", function(e) {
                        ...
                        $(".schedule-form").attr("action", "<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=block-schedule&org=<?php echo $ORGANISATION_ID; ?><?php echo isset($PROCESSED["id"]) ? "&id=" . $PROCESSED["id"] : ""; ?>&cperiod_id=<?php echo $CPERIOD_ID; ?>");
                    });
                });
            </script>
            <form action="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=<?php echo $SECTION; ?>&org=<?php echo $ORGANISATION_ID; ?><?php echo isset($PROCESSED["id"]) ? "&id=" . $PROCESSED["id"] : ""; ?><?php echo isset($PROCESSED["schedule_parent_id"]) ? "&schedule_parent_id=" . $PROCESSED["schedule_parent_id"] : ""; ?>&cperiod_id=<?php echo $CPERIOD_ID; ?>" method="POST" class="form-horizontal schedule-form">
                ...
                if (count($children) <= 0 && $schedule->getScheduleType() == "stream") { ?>
                <div class="control-group">
                    <label for="first-block-end-date" class="control-label" data-toggle="tooltip" title="<?php echo $translate->_("This is an optional date used when auto-generating blocks.\n\n If the curriculum period starts on an odd day such as a Saturday, you may need to force the first block in the set to end on the desired date."); ?>"><?php echo $translate->_("First Block End Date"); ?><i class="icon-question-sign"></i></label>
                    <div class="controls">
                        <div class="input-append">
                            <input id="first-block-end-date" name="first_block_end_date" type="text" class="datepicker span8" />
                ...
                <div class="row-fluid">
                    ...
                    <a href="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=edit-schedule&org=<?php echo $ORGANISATION_ID; ?>&id=<?php echo $PROCESSED["schedule_parent_id"]; ?>&cperiod_id=<?php echo $CPERIOD_ID; ?>" class="btn btn-default"><?php echo $translate->_("Back"); ?></a>
                    <?php } ?>
                ...
                if (EDIT_SCHEDULE === true) {
                    switch ($schedule->getScheduleType()) {
                        ...
                        case "stream" :
                            ...
                            <table id="schedule-table" class="table table-striped table-bordered">
                                ...
                                if ($children) {
                                    foreach ($children as $child) {
                                        $url = ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=edit-schedule&org=" . $ORGANISATION_ID . "&id=" . $child->getID() . "&cperiod_id=" . $CPERIOD_ID;
                                        ?>
                                        <tr>
                            ...
                            <div class="row-fluid">
                                <?php
                                    $url = ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=edit-schedule&org=" . $ORGANISATION_ID . "&id=" . $schedule->getScheduleParentID() . "&cperiod_id=" . $CPERIOD_ID   ;
                                    if (!$schedule->getScheduleParentID()) {
                                        $url = ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=block-schedule&org=" . $ORGANISATION_ID . "&cperiod_id=" . $CPERIOD_ID . "&type_id=" . $cperiod->getCurriculumTypeID();
                                    }
                                ?>
                                <a href="<?php echo $url; ?>" class="btn btn-default"><?php echo $translate->_("Back"); ?></a>
                                <a href="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=add-schedule&org=<?php echo $ORGANISATION_ID; ?>&schedule_parent_id=<?php echo (isset($PROCESSED["id"]) && $PROCESSED["id"] ? $PROCESSED["id"] : 0); ?>&cperiod_id=<?php echo $CPERIOD_ID; ?>" class="btn btn-primary pull-right"><?php echo $translate->_("Add"); ?></a>
                            </div>
                            <?php
                        break;
                    }
