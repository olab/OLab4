 ...
 *
 * Allows administrators to add apartments to the entrada_clerkship.apartments table.
 *
			...
			if (!$ERROR) {
				...
				if (($db->AutoExecute(CLERKSHIP_DATABASE.".apartments", $PROCESSED, "INSERT")) && ($apartment_id = $db->Insert_Id())) {
					...
					*/
					...
					application_log("error", "Failed to insert a new apartment into the database. Database said: ".$db->ErrorMsg());
				}
			}
...
//						alert("Sorry, we were unable to geocode that address");
			...
			<form action="<?php echo ENTRADA_URL; ?>/admin/regionaled/apartments?section=add" method="post" id="addApartmentForm">
				...
				<table style="width: 100%" cellspacing="0" border="0" cellpadding="2" summary="Add Apartment Form">
					<colgroup>
						<col style="width: 3%" />
						<col style="width: 24%" />
						<col style="width: 32%" />
