 ...
 *
 * Allows administrators to add regions to the entrada_clerkship.regions table.
 *
...
if (!defined("IN_REGIONS")) {
	...
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/admin/regionaled/regions?section=add", "title" => "Add Region");
		...
		case 2 :
			...
			if (!$ERROR) {
				...
				if (($db->AutoExecute(CLERKSHIP_DATABASE.".regions", $PROCESSED, "INSERT")) && ($region_id = $db->Insert_Id())) {
					$SUCCESS++;
					$SUCCESSSTR[] = "You have successfully added <strong>".html_encode($PROCESSED["region_name"])."</strong> to the system.<br /><br />You will now be redirected to the Manage Regions index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".ENTRADA_URL."/admin/regionaled/regions\" style=\"font-weight: bold\">click here</a> to continue.";

					$ONLOAD[] = "setTimeout('window.location=\\'".ENTRADA_URL."/admin/regionaled/regions\\'', 5000)";

					application_log("success", "New region [".$region_id."] was added to the system.");
				} else {
					$ERROR++;
					$ERRORSTR[]	= "We were unable to add this region to the system at this time.<br /><br />The system administrator has been notified of this issue, please try again later.";

					application_log("error", "Failed to insert a new region into the database. Database said: ".$db->ErrorMsg());
				}
			}
	...
	switch ($STEP) {
		...
		default :
			...
			<form action="<?php echo ENTRADA_URL; ?>/admin/regionaled/regions?section=add" method="post" id="addRegionForm">
