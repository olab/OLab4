...
if ($objectives) {
	...
	?>
	<a name="evaluation-objectives-section"></a>
	...
	<div id="evaluation-objectives-section">
		...
		$explicit_evaluation_objectives = false;//array();
		$mapped_evaluation_objectives = array();
		if ($mapped_objectives) {
			foreach ($mapped_objectives as $objective) {
				if ($objective["mapped"]) {
					$explicit_evaluation_objectives[] = $objective;
				} else {
					$flat_objectives[] = $objective;
				}

				if ($objective["mapped"]) {
					$mapped_evaluation_objectives[] = $objective;
				}
			}
		}
		?>
		...
		<div class="mapped_objectives right droppable" id="mapped_objectives" data-resource-type="evaluation">
			...
			<p class="well well-small content-small" id="alternate_objective_notice" style="display: none;">
				<strong>Helpful Tip:</strong> Select an objective set from the list on the left and it will expand to show objectives from that set. Then, click an objective from that list to further expand, or if it has no children, it will be reported on. Alternatively, click the checkbox to the right of an objective at any level to map it.
			</p>
		<?php
			if($flat_objectives){
			?>
				...
				<div id="clinical-objective-list">
					...
					<?php
						...
						<li class = "mapped-objective"
							...
							<div class="evaluation-objective-controls">
			...
			<div id="evaluation-list-wrapper" <?php echo ($explicit_evaluation_objectives)?'':' style="display:none;"';?>>
				<a name="evaluation-objective-list"></a>
				<h2 id="evaluation-toggle"  title="Evaluation Question Objective List" class="list-heading">Evaluation Question Specific Objectives</h2>
				<div id="evaluation-objective-list">
					<ul class="objective-list mapped-list" id="mapped_evaluation_objectives" data-importance="evaluation">
					<?php
						if ($explicit_evaluation_objectives) {
							foreach($explicit_evaluation_objectives as $objective){
									$title = ($objective["objective_code"]?$objective["objective_code"].': '.$objective["objective_name"]:$objective["objective_name"]);
								?>
								<li class = "mapped-objective"
									id = "mapped_objective_<?php echo $objective["objective_id"]; ?>"
			...
			<?php
				if ($mapped_evaluation_objectives) {
					foreach($mapped_evaluation_objectives as $objective){
