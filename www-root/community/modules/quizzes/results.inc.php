...
if ($RECORD_ID) {
	...
	if ($quiz_record) {
		...
		if (($is_administrator) || ($quiz_record["proxy_id"] == $ENTRADA_USER->getActiveId())) {
			...
			$BREADCRUMB[]	= array("url" => ENTRADA_URL."/community".$quiz_record["community_url"].":".$quiz_record["page_url"]."?section=results".($QUIZ_TYPE == "community_page" ? "&community=true" : "")."&id=".$RECORD_ID, "title" => limit_chars($quiz_record["quiz_title"], 32));
			...
			application_log("error", "Someone attempted to review results of qprogress_id [".$RECORD_ID."] that they were not entitled to view.");

			header("Location: ".ENTRADA_URL."/community".$COMMUNITY_URL.":".$PAGE_URL);
			exit;
		}
	} else {
		$ERROR++;
