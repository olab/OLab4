			...
			if (galleries_module_access($photo_record["cgallery_id"], "add-comment")) {
						...
						if (!$ERROR) {
							...
							if ($db->AutoExecute("community_gallery_comments", $PROCESSED, "INSERT")) {
								if ($COMMENT_ID = $db->Insert_Id()) {
									$SUCCESS++;
									Entrada_Utilities_Flashmessenger::addMessage(sprintf($translate->_("You have successfully added <strong>%s</strong>."), $PROCESSED["comment_title"]), "success", $MODULE);

									add_statistic("community:".$COMMUNITY_ID.":galleries", "comment_add", "cgcomment_id", $COMMENT_ID);
				...
				switch($STEP) {
					...
					<form action="<?php echo COMMUNITY_URL.$COMMUNITY_URL.":".$PAGE_URL; ?>?section=add-comment&amp;id=<?php echo $RECORD_ID; ?>&amp;step=2" method="post">
						<table summary="Add Photo Comment">
							<colgroup>
								<col style="width: 20%" />
								<col style="width: 80%" />
							</colgroup>
							...
							<tbody>
								...
								<tr>
									...
									<td><input type="text" id="comment_title" name="comment_title" value="<?php echo ((isset($PROCESSED["comment_title"])) ? html_encode($PROCESSED["comment_title"]) : ""); ?>" maxlength="128" style="width: 300px;" /></td>
								</tr>
								<tr>
									<td colspan="2">
