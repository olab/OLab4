        ...
        if (@file_put_contents(CACHE_DIRECTORY . "/import_draft.lck", "L_O_C_K")) {
            ...
            if ($drafts) {
                ...
                foreach ($drafts as $draft) {
                    ...
                    if ($draft["copy_resources_as_draft"]) {
                        $copy_resource_as_draft = $draft["copy_resources_as_draft"];
                    } else {
                        $copy_resource_as_draft = 0;
                    }
                    $msg[$draft["draft_id"]][] = "Imported draft: \"" . $draft["draft_title"] . "\" on " . date("Y-m-d H:i", time());
                    $notification_events = "";
                    ...
                    if ($events) {
                        ...
                        foreach ($events as $event) {
                            ...
                            if (isset($event["draft_parent_id"])) {
                                if ($has_event_id) {
                                    $key = array_search($event["draft_parent_id"], array_map(function ($element) {
                                        return $element['devent_id'];
                                    }, $old_events));
                                    if ($key !== false) {
                                        $event["parent_id"] = $old_events[$key]["new_event_id"];
                                    }
                                } else {
                                    $event["parent_id"] = $old_events[$event["draft_parent_id"]]["new_event_id"];
                                }
                            } else if (isset($event["parent_id"]) && in_array($event["parent_id"], $old_events[$event["parent_id"]])) {
