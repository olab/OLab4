				...
				if ((int) $file_record["file_active"]) {
					if (shares_file_module_access($RECORD_ID, "move-file")) {
						if ($db->AutoExecute("community_share_files", array("cshare_id" => $share_id, "updated_date" => time(), "updated_by" => $ENTRADA_USER->getID()), "UPDATE", "`csfile_id` = ".$db->qstr($RECORD_ID)." AND `cshare_id` = ".$db->qstr($file_record["cshare_id"])." AND `community_id` = ".$db->qstr($COMMUNITY_ID))) {
                            ...
                            Entrada_Utilities_Flashmessenger::addMessage(sprintf($translate->_("You have successfully moved <strong>%s</strong> to <strong>%s</strong>."), $file_record["file_title"], $share_record["folder_title"]), "success", $MODULE);
                        } else {
                            Entrada_Utilities_Flashmessenger::addMessage(sprintf($translate->_("Failed to move <strong>%s</strong> to <strong>%s</strong>."), $file_record["file_title"], $share_record["folder_title"]), "error", $MODULE);
                            application_log("error", "Failed to move [".$RECORD_ID."] file to folder. Database said: ".$db->ErrorMsg());
						}
					}
				} else {
                    Entrada_Utilities_Flashmessenger::addMessage(sprintf($translate->_("The provided file <strong>%s</strong> is deactivated."), $file_record["file_title"]), "error", $MODULE);
