        ...
        if (isset($entry) && $entry != false) {
            ...
            if (isset($rotation) && $rotation != false) {
                $HEAD[] = (
                    ...
                    html_encode(APPLICATION_VERSION) .
                    "\"></script>\n"
                );
                ...
                $BREADCRUMB[] = array(
                    "url" => (
                        ENTRADA_URL . "/clerkship/dutyhours/rotation?id=" .
                        $rotation->getID() . "&cperiod_id=" . $entry->getCurriculumPeriodID()
                    ),
                    "title" => $rotation->getCourseName()
                );
                $BREADCRUMB[] = array("title" => "Edit Log Entry");
                echo "<h2>Edit Duty Hours for " . $rotation->getCourseName() . "</h2>";
                $view_data = [
