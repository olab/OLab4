...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($PEER_REVIEWED_PAPERS_ID) {
		...
		if($result) {
			...
			echo "<h1>Edit Peer-Reviewed Publication</h1>\n";
					...
					if(!$ERROR) {
						...
						if($db->AutoExecute("ar_peer_reviewed_papers", $PROCESSED, "UPDATE", "`peer_reviewed_papers_id`=".$db->qstr($PEER_REVIEWED_PAPERS_ID))) {
								switch($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["post_action"]) {
									case "new" :
										$url = ENTRADA_URL."/annualreport/research?section=add_peer_reviewed";
										$msg	= "You will now be redirected to add more Peer-Reviewed Publication; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.";
									break;
									case "index" :
									default :
								...
								application_log("success", "Edited Peer-Reviewed Publication [".$PEER_REVIEWED_PAPERS_ID."] in the system.");					
		
						} else {
							$ERROR++;
							$ERRORSTR[] = "There was a problem editing this Peer-Reviewed Publication record in the system. The MEdIT Unit was informed of this error; please try again later.";
		
							application_log("error", "There was an error editing the Peer-Reviewed Publication. Database said: ".$db->ErrorMsg());
						}
					} else {
						$STEP = 1;
					}
			...
			switch($STEP) {
				...
				default :
					...
					{
						$peerReviewedPaperQuery = "SELECT * FROM `ar_peer_reviewed_papers` WHERE `peer_reviewed_papers_id` =".$db->qstr($PEER_REVIEWED_PAPERS_ID);
						$PROCESSED = $db->GetRow($peerReviewedPaperQuery);
					}
					if($ERROR) {
						echo display_error();
					}
					?>
					<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=edit_peer_reviewed&amp;step=2&amp;rid=<?php echo $PEER_REVIEWED_PAPERS_ID;?>"" method="post">
					...
					<tr>
						...
						<option value = ""></option>
						<?php
							$types = getPublicationTypesSpecific("Peer-Reviewed");
			...
			application_log("notice", "Failed to provide a valid Peer-Reviewed Publication record identifer when attempting to edit a Peer-Reviewed Paper record.");
		}
	} else {
		$ERROR++;
