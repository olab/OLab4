...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/research?section=add_non_peer_reviewed", "title" => "Add Non-Peer-Reviewed Research");
	...
	echo "<h1>Add Non-Peer-Reviewed Publication</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_non_peer_reviewed_papers", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						application_log("success", "Non-Peer-Reviewed Article [".$EVENT_ID."] added to the system.");					

				} else {
					$ERROR++;
					$ERRORSTR[] = "There was a problem inserting this Non-Peer-Reviewed Article into the system. The MEdIT Unit was informed of this error; please try again later.";

					application_log("error", "There was an error inserting a Non-Peer-Reviewed Article. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="non_peer_reviewed_papers">
				<colgroup>
					<col style="width: 100%" />
				</colgroup>
				<tr>
					<td>
						<div id="pubmed_id_div" style="display: block">
							<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=add_non_peer_reviewed&step=1" method="post">	
							<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Non-Peer-Reviewed Publication">
								<colgroup>
									<col style="width: 3%" />
									<col style="width: 20%" />
									<col style="width: 77%" />
						...
						<div id="pubmed_id_results" style="display: block">
							<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=add_non_peer_reviewed&step=2" method="post">
