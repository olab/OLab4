...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/research?section=add_poster_reports", "title" => "Add Posters / Reports");
	...
	echo "<h1>Add Poster Presentations / Technical Reports</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_poster_reports", $PROCESSED, "INSERT")) {
			...
			if(isset($_POST['pubmed_id'])) {
				...
				if(!isset($xml->DocSum->Item[3]) || $xml->DocSum->Item[3] == "") {
					...
					if(isset($xml->DocSum->Item[15]) && $xml->DocSum->Item[15] == "ppublish+epublish") {
						$source = $xml->DocSum->Item[22];
					} else {
						if($xml->DocSum->Item[21] != "0") {
							$source = $xml->DocSum->Item[21];
						} else {
							$source = $xml->DocSum->Item[22];
						}
					}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="poster_reports">
				<colgroup>
					<col style="width: 100%" />
				</colgroup>
				<tr>
					<td>
						<div id="pubmed_id_div" style="display: block">
							<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=add_poster_reports&step=1" method="post">	
							<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Poster Presentations / Technical Reports">
								<colgroup>
									<col style="width: 3%" />
									<col style="width: 20%" />
									<col style="width: 77%" />
						...
						<div id="pubmed_id_results" style="display: block">
							<form action="<?php echo ENTRADA_URL; ?>/annualreport/research?section=add_poster_reports&step=2" method="post">
							...
							<tr>
								...
								<td><input type="text" id="title" name="title" value="<?php echo (isset($PROCESSED["title"]) ? html_encode($PROCESSED["title"]) : html_encode($xml->DocSum->Item[5])); ?>" maxlength="255" style="width: 95%" /></td>
							</tr>
							<tr>
								<td></td>
								<td style="vertical-align: top"><label for="source" class="form-required">Source / Presented @</label></td>
								<td><input type="text" id="source" name="source" value="<?php echo (isset($PROCESSED["source"]) ? html_encode($PROCESSED["source"]) : html_encode($source)); ?>" maxlength="255" style="width: 95%" /></td>
							</tr>
							<tr>
								<td></td>
								<td style="vertical-align: top"><label for="author_list" class="form-required">Author List</label></td>
								<td><input type="text" id="author_list" name="author_list" value="<?php echo (isset($PROCESSED["author_list"]) ? html_encode($PROCESSED["author_list"]) : html_encode($authorList)); ?>" maxlength="255" style="width: 95%" /></td>
							</tr>
							<tr>
								<td></td>
								<td style="vertical-align: top"><label for="editor_list" class="form-nrequired">Editor List</label></td>
								<td><input type="text" id="editor_list" name="editor_list" value="<?php echo (isset($PROCESSED["editor_list"]) ? html_encode($PROCESSED["editor_list"]) : ""); ?>" maxlength="255" style="width: 95%" /></td>
							</tr>
							<tr>
								<td></td>
							...
							<tr>
								...
								<td><input type="text" id="volume" name="volume" value="<?php echo (isset($PROCESSED["volume"]) ? html_encode($PROCESSED["volume"]) : html_encode($xml->DocSum->Item[6])); ?>" maxlength="10" style="width: 15%" /></td>
							</tr>
							<tr>
								<td></td>
								<td style="vertical-align: top"><label for="edition" class="form-nrequired">Edition</label></td>
								<td><input type="text" id="edition" name="edition" value="<?php echo (isset($PROCESSED["edition"]) ? html_encode($PROCESSED["edition"]) : html_encode($xml->DocSum->Item[7])); ?>" maxlength="10" style="width: 15%" /></td>
							</tr>
							<tr>
								<td></td>
								<td style="vertical-align: top"><label for="pages" class="form-nrequired">Pages</label></td>
								<td><input type="text" id="pages" name="pages" value="<?php echo (isset($PROCESSED["pages"]) ? html_encode($PROCESSED["pages"]) : html_encode($xml->DocSum->Item[8])); ?>" maxlength="10" style="width: 15%" /></td>
							</tr>
							<tr>
								<td></td>
