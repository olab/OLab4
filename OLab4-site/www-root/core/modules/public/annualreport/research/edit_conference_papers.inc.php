...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($CONFERENCE_PAPERS_ID) {
		$query	= "SELECT * FROM `ar_conference_papers` WHERE `conference_papers_id`=".$db->qstr($CONFERENCE_PAPERS_ID)." AND `proxy_id` = ".$db->qstr($ENTRADA_USER->getActiveId());
						...
						<td colspan="3" style="padding-top: 25px">
							...
							<tr>
								<td style="width: 25%; text-align: left">
									<input type="button" class="btn" value="Cancel" onclick="window.location='<?php echo ENTRADA_URL; ?>/annualreport/research#conference_papers'" />
								</td>
