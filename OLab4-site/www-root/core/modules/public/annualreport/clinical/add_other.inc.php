...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["title"]			= "Add Other Professional Activity";
	$PAGE_META["site"]			= "Other Professional Activity portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/research?section=add_other", "title" => "Add Other Professional Activity");
	
	// This grid should be expanded upon redirecting back to the clinical index.
	$_SESSION["clinical_expand_grid"] = "other_grid";
	
	echo "<h1>Add Other Professional Activity</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_other_activity", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						application_log("success", "Other Professional Activity [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an Other Professional Activity record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_URL; ?>/annualreport/clinical?section=add_other&amp;step=2" method="post">
			...
			<tr>
				...
				<?php
					...
					foreach($otherLocationArray as $otherLocationListValue) {
						echo "<option value=\"".$otherLocationListValue["other_location"]."\"".(($PROCESSED["site"] == $otherLocationListValue["other_location"]) ? " selected=\"selected\"" : "").">".html_encode($otherLocationListValue["other_location"])."</option>\n";
					}
				?>
				</select>
				</td>
			...
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="average_hours" class="form-required">Average Hours</label></td>				
