...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["title"]			= "Add Other Undergraduate Teaching";
	$PAGE_META["description"]	= "Other Undergraduate Teaching portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	...
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/education?section=add_undergraduate_nonmedical", "title" => "Add Other Undergraduate Teaching");
	
	echo "<h1>Add Other Undergraduate Teaching</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :
			$ENDERROR = false;
			...
			if((isset($_POST["course_number"])) && ($course_number = clean_input($_POST["course_number"], array("notags", "trim")))) {
				...
				if(in_array(strtoupper($PROCESSED["course_number"]), $supervisionCourseArray)) {
					$ERROR++;
					$ERRORSTR[] = "This <strong>Course Number</strong> belongs <a href=\"".ENTRADA_URL."/annualreport/education?section=add_undergrad_sup\"><u>here</u></a>.";
				}
			}
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute(DATABASE_NAME.".ar_undergraduate_nonmedical_teaching", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						switch($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["post_action"]) {
							case "new" :
								$url 	= ENTRADA_URL."/annualreport/education?section=add_undergraduate_nonmedical";
							...
							default :
								...
								$msg	= "You will now be redirected to the annualreport page; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.";
							break;
						}
						...
						application_log("success", "Course Name [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an annualreport record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_URL; ?>/annualreport/education?section=add_undergraduate_nonmedical&amp;step=2" method="post">
			<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Education">
			<colgroup>
				<col style="width: 3%" />
				<col style="width: 20%" />
				<col style="width: 77%" />
			...
			<tr>
				...
				<option value=""></option>
				<?php
					if(isset($PROCESSED["assigned"]))
					{
						if($PROCESSED["assigned"] == "Yes")
						{
			...
			<tr>
				...
				<td><input type="text" id="lec_enrollment" name="lec_enrollment" value="<?php echo html_encode($PROCESSED["lec_enrollment"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="lec_hours" class="form-required">Lecture Hours</label></td>
				<td><input type="text" id="lec_hours" name="lec_hours" value="<?php echo html_encode($PROCESSED["lec_hours"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
				<td><label for="lab_enrollment" class="form-required">Lab Enrollment</label></td>
				<td><input type="text" id="lab_enrollment" name="lab_enrollment" value="<?php echo html_encode($PROCESSED["lab_enrollment"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="lab_hours" class="form-required">Lab Hours</label></td>
				<td><input type="text" id="lab_hours" name="lab_hours" value="<?php echo html_encode($PROCESSED["lab_hours"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
				<td><label for="tut_enrollment" class="form-required">Tutorial Enrollment</label></td>
				<td><input type="text" id="tut_enrollment" name="tut_enrollment" value="<?php echo html_encode($PROCESSED["tut_enrollment"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="tut_hours" class="form-required">Tutorial Hours</label></td>
				<td><input type="text" id="tut_hours" name="tut_hours" value="<?php echo html_encode($PROCESSED["tut_hours"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>			
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="sem_enrollment" class="form-required">Seminar Enrollment</label></td>
				<td><input type="text" id="sem_enrollment" name="sem_enrollment" value="<?php echo html_encode($PROCESSED["sem_enrollment"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="sem_hours" class="form-required">Seminar Hours</label></td>
				<td><input type="text" id="sem_hours" name="sem_hours" value="<?php echo html_encode($PROCESSED["sem_hours"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			...
			<tr>
				...
				<td><input type="text" id="coord_enrollment" name="coord_enrollment" value="<?php echo html_encode($PROCESSED["coord_enrollment"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<?php } ?>
			<tr>
				<td></td>
				<td style="vertical-align: top"><label for="pbl_hours" class="form-required">PBL Hours</label></td>
				<td><input type="text" id="pbl_hours" name="pbl_hours" value="<?php echo html_encode($PROCESSED["pbl_hours"]); ?>" maxlength="5" style="width: 40px" /></td>
			</tr>
			<tr>
				<td></td>
