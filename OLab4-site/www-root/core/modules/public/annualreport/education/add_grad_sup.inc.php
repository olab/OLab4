...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$PAGE_META["title"]			= "Add Graduate Supervision";
	$PAGE_META["description"]	= "Graduate Supervision portion of your annual report should be entered / located here.";
	$PAGE_META["keywords"]		= "";
	...
	$BREADCRUMB[]	= array("url" => ENTRADA_URL."/annualreport/education?".replace_query(array("section" => "add_grad_sup")), "title" => "Add Graduate Supervision");
	
	echo "<h1>Add Graduate Supervision</h1>";
	// Error Checking
	switch($STEP) {
		case 2 :			
			/**
			...
			if(!$ERROR) {
				...
				if($db->AutoExecute("ar_graduate_supervision", $PROCESSED, "INSERT")) {
					$EVENT_ID = $db->Insert_Id();
						...
						$SUCCESS++;
						$SUCCESSSTR[]  = "You have successfully added information pertaining to supervision of graduate student <strong>".html_encode($PROCESSED["student_name"])."</strong> to the system.<br /><br />".$msg;
						$ONLOAD[]		= "setTimeout('window.location=\\'".$url."\\'', 5000);";

						application_log("success", "Graduate Supervision [".$EVENT_ID."] added to the system.");					
					...
					application_log("error", "There was an error inserting an graduate supervision record. Database said: ".$db->ErrorMsg());
				}
			} else {
				$STEP = 1;
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_URL; ?>/annualreport/education?section=add_grad_sup&amp;step=2" method="post">
			<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Graduate Supervision">
			<colgroup>
				<col style="width: 3%" />
				<col style="width: 20%" />
				<col style="width: 77%" />
			...
			<tr>
				...
				<option value=""></option>
				<?php
					echo "<option value=\"Active\"".(($PROCESSED["active"] == "Active") ? " selected=\"selected\"" : "").">Active</option>\n";
					echo "<option value=\"Inactive\"".(($PROCESSED["active"] == "Inactive") ? " selected=\"selected\"" : "").">Inactive</option>\n";
					echo "</select>";
				?>
				</td>
			</tr>
			...
			<tr>
				...
				<option value=""></option>
				<?php
					echo "<option value=\"Yes\"".(($PROCESSED["thesis_defended"] == "Yes") ? " selected=\"selected\"" : "").">Yes</option>\n";	
					echo "<option value=\"No\"".(($PROCESSED["thesis_defended"] == "No") ? " selected=\"selected\"" : "").">No</option>\n";
					echo "</select>";
				?>
				</td>
			</tr>
