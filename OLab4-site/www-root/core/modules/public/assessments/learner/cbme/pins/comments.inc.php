...
if(!defined("PARENT_INCLUDED")) {
    ...
    if (!$ERROR) {
        ...
        $cbme_progress_api = new Entrada_CBME_Visualization(array(
            ...
            "filters" => $_GET,
            "query_limit" => 12,
            "limit_dataset" => array("assessment_comment_pins", "total_pinned_assessment_comment_count", "filtered_pinned_assessment_comment_count", "course_epas", "roles", "course_milestones", "course_assessment_tools", "rating_scales", "filter_list_data", "course_stages", "unread_assessment_count"),
            "courses" => $courses,
        ...
        if ($learner) {
            ...
            $cbme_assessment_comments_view->render(array(
                "assessments" => $dataset["assessment_comment_pins"],
                "total_count" => $dataset["total_pinned_assessment_comment_count"],
                "filtered_count" => $dataset["filtered_pinned_assessment_comment_count"],
                ...
                "form_action_url" => ENTRADA_URL . "/assessments/learner/cbme/pins/comments?proxy_id=" . html_encode($PROCESSED["proxy_id"]),
                "form_reset_url" => ENTRADA_URL . "/assessments/learner/cbme/pins/comments?proxy_id=" . html_encode($PROCESSED["proxy_id"]),
