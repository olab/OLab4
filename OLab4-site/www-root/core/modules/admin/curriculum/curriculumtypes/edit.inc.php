...
if (!defined("PARENT_INCLUDED") ) {
	...
	$BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/curriculum/curriculumtypes?".replace_query(array("section" => "add"))."&amp;org=".$ORGANISATION_ID, "title" => "Edit");
		...
		case 2 :
			...
			if (!$ERROR) {
				...
				if ($db->AutoExecute("curriculum_lu_types", $PROCESSED, "UPDATE","`curriculum_type_id`=".$db->qstr($PROCESSED["curriculum_type_id"]))) {
					...
					$url = ENTRADA_URL . "/admin/curriculum/curriculumtypes?org=".$ORGANISATION_ID;
					$SUCCESS++;
	...
	switch ($STEP) {
		...
		default:
			...
			?>
        <form class="form-horizontal" action="<?php echo ENTRADA_URL."/admin/curriculum/curriculumtypes"."?".replace_query(array("action" => "add", "step" => 2))."&org=".$ORGANISATION_ID; ?>" id ="curriculum_form" method="post">
            ...
            <div class="control-group">
                <div class="pull-right">
                    <button type="button" class="btn" onclick="window.location='<?php echo ENTRADA_URL; ?>/admin/curriculum/curriculumtypes?org=<?php echo $ORGANISATION_ID;?>'"><?php echo $translate->_("Cancel"); ?></button>
                    ...
                    if ($PROCESSED["periods"]) {
                        foreach ($PROCESSED["periods"] as $currentIdx => $period) {
                            ...
                            <tr id="period_<?php echo $currentIdx;?>" class="curriculum_period">
                                ...
                                <td><a href="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=block-schedule&org=<?php echo $ORGANISATION_ID; ?>&cperiod_id=<?php echo clean_input($currentIdx, "numeric"); ?>&type_id=<?php echo $PROCESSED["curriculum_type_id"]; ?>">Block Schedule</a></td>
                            </tr>
                            <?php
                        }
                    }
