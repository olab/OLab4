...
if ((!defined("PARENT_INCLUDED"))) {
    ...
    if ($curriculum_period) {
        ...
        if ($curriculum_type) {
            $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/".$MODULE."/".$SUBMODULE."?".replace_query(array("section" => "edit"))."&amp;org=".$ORGANISATION_ID."&amp;type_id=".$PROCESSED["type_id"], "title" => $curriculum_type->getCurriculumTypeName());
            $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/".$MODULE."/".$SUBMODULE."?".replace_query(array("section" => "edit"))."&amp;org=".$ORGANISATION_ID."&amp;type_id=".$PROCESSED["type_id"], "title" => "Block Schedule");
            ...
            switch ($STEP) {
                case 3 :
                    ...
                    break;
                case 2 :
                    add_notice("You have selected the following block schedules to be deleted, please confirm below if you wish to proceed.  Otherwise, you may <a href=\"" . ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "/curriculumtypes?" . replace_query(array("section" => "edit")) . "&amp;org=" . $ORGANISATION_ID . "&amp;type_id=" . $curriculum_type->getID() . "\">cancel</a>.");
                    break;
                case 1 :
                default:
                    break;
            ...
            <?php } ?>
            <form action="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=block-schedule&org=<?php echo $ORGANISATION_ID; ?>&cperiod_id=<?php echo $PROCESSED["cperiod_id"]; ?>" method="POST">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    if ($schedules) {
                        foreach ($schedules as $schedule) {
                            ...
                            $url = ENTRADA_URL."/admin/".$MODULE."/".$SUBMODULE."?section=edit-schedule&org=".$ORGANISATION_ID."&id=".$schedule->getID()."&cperiod_id=".$PROCESSED["cperiod_id"];;
                            ?>
                            <tr>
                ...
                <div class="row-fluid space-above">
                    ...
                    <a href="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>/<?php echo $SUBMODULE; ?>?section=add-schedule&org=<?php echo $ORGANISATION_ID; ?>&schedule_parent_id=0&cperiod_id=<?php echo $PROCESSED["cperiod_id"]; ?>" class="btn btn-primary pull-right"><?php echo $translate->_("New"); ?></a>
                </div>
            </form>
            <?php
        }
    } else {
        echo display_error(sprintf($translate->_("An invalid curriculum type was selected, please <a href=\"%s\">click here</a> to return to the curriculum layout page."), ENTRADA_URL . "/admin/" . $MODULE . "/" . $SUBMODULE . "?section=edit&amp;org=" . $ORGANISATION_ID . "&amp;type_id=" . $PROCESSED["type_id"]));
    }
}
