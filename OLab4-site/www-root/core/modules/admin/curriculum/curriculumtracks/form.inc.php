...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_CURRICULUM_TRACKS"))) {
    exit;
} elseif (!isset($_SESSION["isAuthorized"]) || !(bool) $_SESSION["isAuthorized"]) {
    header("Location: ".ENTRADA_URL);
    exit;
} elseif (!$ENTRADA_ACL->amIAllowed("curriculum", "update", false)) {
                ...
                if (defined("ADD_TRACK")) {
                    ...
                    if ($insert_track = $curriculum_track->fromArray($PROCESSED)->insert()) {
                        if ($result_connection = Models_Curriculum_Track::insertTrackOrgRelationship($insert_track->getID(),$ORGANISATION_ID)) {
                            add_success("Successfully added the Curriculum Track [<strong>". $curriculum_track->getCurriculumTrackName()."</strong>]. You will now be redirected to the Curriculum Tracks index, please <a href=\"".ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID."\">click here</a> if you do not wish to wait.");
                        } else {
                            //$ONLOAD[]	= "setTimeout('window.location=\\'".ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID."\\'', 5000)";
                            add_error("An error occurred while attempting to add the Curriculum Track [<strong>".  $curriculum_track->getCurriculumTrackName()."</strong>]. A system administrator has been informed, please try again later.<br /><br />You will now be redirected to the Curriculum Tracks index, please <a href=\"".ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID."\">click here</a> if you do not wish to wait.");
                        }
                    ...
                    if (isset($RECORD_ID)) {
                        if ($curriculum_track->update()) {
                            add_success("Successfully updated the Curriculum Track [<strong>". $curriculum_track->getCurriculumTrackName()."</strong>]. You will now be redirected to the Curriculum Track index, please <a href=\"".ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID."\">click here</a> if you do not wish to wait.");
                        } else {
                          //  $ONLOAD[]	= "setTimeout('window.location=\\'".ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID."\\'', 5000)";
    ...
    switch ($STEP) {
        ...
        default :
            ...
            ?>
            <form action="<?php echo ENTRADA_URL . "/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID; ?>&section=<?php echo defined("EDIT_TRACK") ? "edit&id=".$RECORD_ID : "add"; ?>&step=2" method="POST" class="form-horizontal"  enctype="multipart/form-data">
                ...
                <div id="track-details-section">
                    ...
                    <div class="row-fluid">
                        <a href="<?php echo ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID; ?>" class="btn">Cancel</a>
