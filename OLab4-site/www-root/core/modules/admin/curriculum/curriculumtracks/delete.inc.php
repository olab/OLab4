...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CURRICULUM_TRACKS"))) {
    ...
    exit;
} elseif (!$ENTRADA_ACL->amIAllowed("curriculum", "delete", false)) {
    ...
    $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID."&section=delete&id=".$RECORD_ID, "title" => "Delete Evaluation Response Descriptors");
    ...
    switch ($STEP) {
        ...
        default :
            ...
            if (isset($curriculum_tracks) && is_array($curriculum_tracks)) { ?>
                ...
                <form action="<?php echo ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID; ?>&section=delete&step=2" method="POST" id="curriculumtracks-list">
                    <table class="table table-striped table-bordered" width="100%" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <?php foreach ($curriculum_tracks as $curriculum_track) { ?>
                            <tr class="descriptor" data-id="<?php echo html_encode($curriculum_track->getID()); ?>">
                                ...
                                <td class="name"><a href="<?php echo ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID; ?>&section=edit&id=<?php echo html_encode($curriculum_track->getID()); ?>"><?php echo html_encode($curriculum_track->getCurriculumTrackName()); ?></a></td>
                            </tr>
                        <?php } ?>
                        </tbody>
                    </table>
                    <div class="row-fluid">
                        <a href="<?php echo ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID; ?>" class="btn" role="button">Cancel</a>
                        <input type="submit" class="btn btn-primary pull-right" value="Delete" />
                    </div>
                </form>
            <?php } else { ?>
                <div class="alert alert-info">No Curriculum Tracks have been selected to be deleted. Please <a href="<?php echo ENTRADA_URL."/admin/curriculum/curriculumtracks?org=".$ORGANISATION_ID; ?>">click here</a> to return to the Curriculum Track index.</div>
            <?php }
