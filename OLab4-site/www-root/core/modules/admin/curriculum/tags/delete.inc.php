...
if (!defined("PARENT_INCLUDED") || !defined("IN_OBJECTIVES")) {
	...
	if ($MODE == "ajax" && $OBJECTIVE_ID) {
		...
		switch($STEP) {
				...
				if ($_POST["confirm"] == "on") {
					...
					if ($objectives) {
						foreach ($objectives as $objective) {
							...
							*/
							$query = "DELETE FROM `objective_organisation` WHERE `organisation_id` = ".$db->qstr($ENTRADA_USER->getActiveOrganisation())." AND `objective_id` = ".$db->qstr($objective["objective_id"]);
							if (!$db->Execute($query)) {
						...
						deactivate_objective_children($OBJECTIVE_ID, $ENTRADA_USER->getActiveOrganisation());
			...
			default :
				...
				if ($objective) {
					...
					<form id="objective-form" action="<?php echo ENTRADA_URL."/admin/curriculum/objectives?".replace_query(array("step" => "2")); ?>" method="post">
		...
		$redirect_url = ENTRADA_URL . "/admin/curriculum/objectives";
		...
		if (!$total_objectives) {
			header("Location: " . ENTRADA_URL . "/admin/curriculum/objectives");
			exit;
		}
		...
		switch ($STEP) {
			...
			default :
				if (has_error()) {
					...
					?>
					<form action="<?php echo ENTRADA_URL."/admin/curriculum/objectives?section=delete"; ?>&amp;step=2" method="post">
						...
						<table class="table table-striped" summary="Curriculum Tag Sets For Deactivation">
							...
							foreach ($objectives as $result) {
								...
								echo"	<td><a href=\"".ENTRADA_URL."/admin/curriculum/objectives?section=edit&amp;id=".$result["objective_id"]."\">" . html_encode($result["objective_name"]) . "</a></td>";
								echo "</tr>";
							}
							?>
							</tbody>
