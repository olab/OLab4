...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_ROTATION_SCHEDULE"))) {
    ...
    $draft = new Models_Schedule_Draft();

    $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/" . $MODULE . "?section=drafts", "title" => $translate->_("My Drafts"));
    $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/" . $MODULE . "?section=add", "title" => $translate->_("Create New Draft"));

    $SECTION_TEXT = $MODULE_TEXT[$SECTION];
    define("ADD_SCHEDULE", true);
    ?>
    <h1><?php echo $translate->_("Add Draft"); ?></h1>
    <?php
