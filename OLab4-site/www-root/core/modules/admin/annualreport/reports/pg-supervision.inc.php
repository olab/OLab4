...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	$BREADCRUMB[]	= array("url" => "", "title" => "Graduate Supervision Report" );
	...
	if(isset($years["start_year"]) && $years["start_year"] != "") {
		...
		if ($STEP == 2) {
			...
			FROM `".DATABASE_NAME."`.`ar_graduate_supervision`, `".AUTH_DATABASE."`.`user_data`, `".AUTH_DATABASE."`.`user_departments`, `".AUTH_DATABASE."`.`departments`
			WHERE `year_reported` = ".$db->qstr($PROCESSED["year_reported"]).$type_where."
			AND `".DATABASE_NAME."`.`ar_graduate_supervision`.`proxy_id` = `".AUTH_DATABASE."`.`user_data`.`id` 
			...
			if ($results) {
				echo "<h2>Annual Report Graduate Supervision data for ".$title_suffix." in " . $departmentString."</h2>";
				...
				foreach ($results as $result) {
					if($multipleDivisions && $prevDepartment != $result["dep_id"]) {
						...
						?>
						<table class="tableList" cellspacing="0" summary="Graduate Supervision Breakdown">
							<colgroup>
								<col style="width: 5%" />
								<col style="width: 1%">
								<col style="width: 1%">
						...
						?>
						<table class="tableList" cellspacing="0" summary="Graduate Supervison Breakdown">
						<colgroup>
							<col style="width: 5%" />
							<col style="width: 1%">
							<col style="width: 1%">
