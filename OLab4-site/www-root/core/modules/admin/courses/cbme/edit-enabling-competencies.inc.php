...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CBME"))) {
    ...
    if (isset($_GET["id"]) && $tmp_input = clean_input($_GET["id"], array("trim", "int"))) {
        ...
        add_error($translate->_("In order to edit Enabling Competencies a valid course identifier must be supplied."));
    }

    $settings = new Entrada_Settings();
    if ((int) $settings->read("cbme_standard_kc_ec_objectives")) {
        add_error($translate->_("This organisation does not support course specific Enabling Competencies."));
    }
    ...
    if (!$ERROR) {
        ...
        if ((int) $course_settings->read("cbme_standard_kc_ec_objectives")) {
            add_error($translate->_("This course does not have course specific Enabling Competencies enabled."));
        }
    }
    ...
    if (!$ERROR) {
        ...
        if ($course && $ENTRADA_ACL->amIAllowed(new CourseContentResource($course->getID(), $course->getOrganisationID()), "update")) {
            ...
            $edit_milestones_view->render(array(
                "course_id" => $PROCESSED["course_id"],
                "objectives" => (!empty($dataset["course_enabling_competencies"]) ? $dataset["course_enabling_competencies"] : $dataset["unmapped_course_enabling_competencies"]),
                "show_secondary_objective" => false,
                "objective_set_name" => $translate->_("Enabling Competencies")
            ));
        } else {
