	...
	switch ($STEP) {
		case 2 :
			...
			if (isset($_POST["resource"]) && ($resource = clean_input($_POST["resource"], array("htmlbrackets", "trim")))) {
                ...
                if (Models_Medbiq_Resource::fetchRowByResource($PROCESSED["resource"])) {
                    add_error(sprintf($translate->_("The Medbiquitous Resource <strong>%s</strong> is already registered in the system."), $PROCESSED["resource"]));
                }
			} else {
                add_error($translate->_("The <strong>Medbiquitous Resource</strong> is a required field."));
			}
			...
			if (!has_error()) {
                $medbiq_resource = new Models_Medbiq_Resource($PROCESSED);
                $medbiq_resource->setResource($PROCESSED["resource"]);
                $medbiq_resource->setResourceDescription($PROCESSED["resource_description"]);
                $medbiq_resource->setActive(1);
                $medbiq_resource->setUpdatedDate(time());
                $medbiq_resource->setUpdatedBy($ENTRADA_USER->getID());

                if ($medbiq_resource->insert()) {
                    $url = ENTRADA_URL . "/admin/settings/manage/medbiqresources?org=" . $ORGANISATION_ID;
                    $ONLOAD[] = "setTimeout('window.location=\\'" . $url . "\\'', 5000)";

                    add_success(sprintf($translate->_("You have successfully added <strong>%s</strong> to the system.<br /><br />You will now be redirected to the Medbiquitous Resources index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"%s\" style=\"font-weight: bold\">click here</a> to continue."), html_encode($PROCESSED["resource"]), $url));

                    application_log("success", "New Medbiquitous Resource [" . $PROCESSED["resource"] . "] added to the system.");
                } else {
                    add_error($translate->_("There was a problem inserting this resource into the system. The system administrator was informed of this error; please try again later."));
		...
		default:	
            ...
            <form action="<?php echo ENTRADA_URL."/admin/settings/manage/medbiqresources"."?".replace_query(array("action" => "add", "step" => 2))."&org=".$ORGANISATION_ID; ?>" method="post">
                <table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Adding Page">
                    ...
                    <thead>
                        <tr>
                            <td colspan="2"><h1><?php echo $translate->_("Add Medbiquitous Resource"); ?></h1></td>
                        </tr>
                    </thead>
                    <tfoot>
                        <tr>
                    ...
                    <tbody>
                        <tr>
                            <td><label for="resource" class="form-required"><?php echo $translate->_("Medbiquitous Resource:"); ?></label></td>
                        ...
                        <tr>
                            <td style="vertical-align: top;"><label for="resource_description" class="form-nrequired"><?php echo $translate->_("Description:"); ?></label></td>
                            <td>
