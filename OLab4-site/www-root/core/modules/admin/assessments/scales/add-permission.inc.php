...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_ITEMS"))) {
    ...
    $item = Models_Assessments_Item::fetchRowByID($PROCESSED["id"]);

    ?>
    <h1><?php echo $translate->_("Permissions") ?></h1>
    <?php

    $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/" . $MODULE . "/" . $SUBMODULE . "?section=edit-item&id=" . $PROCESSED["id"], "title" => $translate->_("Edit Item"));
    $BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/" . $MODULE . "/" . $SUBMODULE . "?section=" . $SECTION . "&id=" . $PROCESSED["id"], "title" => $SECTION_TEXT["breadcrumb"]["title"]);
    ...
    switch ($STEP) {
        case 2:
            ...
            if (isset($PROCESSED) && !empty($PROCESSED["authors"])) {
                ...
                foreach ($PROCESSED["authors"] as $author_type => $authors) {
                    foreach ($authors as $author_id) {
                        $a = Models_Assessments_Form_Author::fetchRowByFormIDAuthorIDAuthorType($PROCESSED["id"], $author_id, $author_type);
                        if ($a) {
                            if ($a->getDeletedDate()) {
            ...
            if ($added <= 0) {
                ...
                $ONLOAD[]	= "setTimeout('window.location=\\'".ENTRADA_URL."/admin/".$MODULE."/".$SUBMODULE."?section=edit-item&id=".$PROCESSED["id"]."\\'', 5000)";
            }
            break;
    }
        ...
        case 1 :
            ...
            if ($PROCESSED["id"]) {
                ...
                if ($item) {
                    ...
                    <form action="<?php echo ENTRADA_URL."/admin/" . $MODULE . "/" . $SUBMODULE . "?section=" . $SECTION; ?>&id=<?php echo $PROCESSED["id"]; ?>" class="form-horizontal" method="POST">
