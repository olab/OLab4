...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_FORMS"))) {
    ...
    if (isset($_SESSION[APPLICATION_IDENTIFIER]["assessments"]["cbmeforms"]["selected_filters"]) && !empty($_SESSION[APPLICATION_IDENTIFIER]["assessments"]["cbmeforms"]["selected_filters"])) {
        $sidebar_html = "";
        foreach ($_SESSION[APPLICATION_IDENTIFIER]["assessments"]["cbmeforms"]["selected_filters"] as $key => $filter_type) {
    ...
    <script type="text/javascript">
        ...
        jQuery(function($) {
            $("#advanced-search").advancedSearch(
                {
                    api_url : "<?php echo ENTRADA_URL . "/admin/assessments/cbmeforms?section=api-forms" ; ?>",
    ...
    <div id="assessment-forms-container">
        <form id="form-table-form" action="<?php echo ENTRADA_URL . "/admin/assessments/cbmeforms?section=delete&step=1"; ?>" method="POST">
        ...
        $add_form_modal->render(
            array (
                ...
                "form_types" => Models_Assessments_Form_Type::fetchAllByOrganisationIDCategory($ENTRADA_USER->getActiveOrganisation(), "cbme_form"),
    ...
    <?php if (isset($_SESSION[APPLICATION_IDENTIFIER]["assessments"]["cbmeforms"]["selected_filters"]) && !empty($_SESSION[APPLICATION_IDENTIFIER]["assessments"]["cbmeforms"]["selected_filters"])): ?>
        <form id="search-targets-form">
            <?php foreach ($_SESSION[APPLICATION_IDENTIFIER]["assessments"]["cbmeforms"]["selected_filters"] as $key => $filter_type): ?>
