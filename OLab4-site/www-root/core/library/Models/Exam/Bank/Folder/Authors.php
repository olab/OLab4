 ...
 *
 * A model for exam folder authors, give permission to the exams and questions in these folders
 * 
...
class Models_Exam_Bank_Folder_Authors extends Models_Base {
    ...
    protected static $table_name = "exam_bank_folder_authors";
    ...
    public function getAuthorName() {
        if (isset($this->author_name)) {
            ...
            switch ($this->author_type) {
                case "proxy_id" :
                    $user = User::fetchRowByID($this->author_id, null, null);
                    if ($user) {
    ...
    /* @return bool|Models_Exam_Bank_Folder_Authors */
    ...
    private static function fetchParentAuthors($folder_id, $type, $author_array, $level, $deleted_date = NULL) {
        ...
        if ($folder_id !== 0) {
            ...
            $parent_folder_array = Models_Exam_Bank_Folder_Authors::fetchAllByFolderIDAuthorType($folder_id, $type, $deleted_date);
            ...
            $current_folder = Models_Exam_Bank_Folders::fetchRowByID($folder_id);
            ...
            if (isset($parent_folder_id) && $parent_folder_id != 0) {
                $author_array = Models_Exam_Bank_Folder_Authors::fetchParentAuthors($parent_folder_id, $type, $author_array, $level, $deleted_date);
    ...
    /* @return bool|array */
    public static function fetchAllInheritedByFolderID($folder_id, $sort = true, array $type_array = array("proxy_id", "organisation_id", "course_id"),  $deleted_date = NULL) {
        global $db;
        ...
        foreach ($type_array as $type) {
            switch ($type) {
                case "proxy_id" :
                    $proxy_array    = Models_Exam_Bank_Folder_Authors::fetchParentAuthors($folder_id, $type, $proxy_array, $level);
                    break;
                case "organisation_id" :
                    $org_array      = Models_Exam_Bank_Folder_Authors::fetchParentAuthors($folder_id, $type, $org_array, $level);
                    break;
                case "course_id" :
                    $course_array   = Models_Exam_Bank_Folder_Authors::fetchParentAuthors($folder_id, $type, $course_array, $level);
                    break;
            }

        }
        //sort the arrays by author name
        usort($proxy_array, 'Models_Exam_Bank_Folder_Authors::sortByAuthorName');
        usort($org_array, 'Models_Exam_Bank_Folder_Authors::sortByAuthorName');
        usort($course_array, 'Models_Exam_Bank_Folder_Authors::sortByAuthorName');
            ...
            case "organisation_id" :
                if (empty($search_value)) {
                    $query = "SELECT a.`organisation_id` AS `id`, `organisation_title` AS `fullname`, '' AS `email`
                                            ...
                                            LEFT JOIN `exam_bank_folder_authors` AS c
                    ...
                    $query = "SELECT a.`organisation_id` AS `id`, a.`organisation_title` AS `fullname`, '' AS `email`
                                            FROM `" . AUTH_DATABASE . "`.`organisations` AS a
                                            LEFT JOIN `exam_bank_folder_authors` AS b
    ...
    public static function fetchByAuthorTypeProxyID ($organisation_id = null, $search_value = null) {
        ...
        $query = "  SELECT a.`folder_id`, b.*, c.`id`, c.`firstname`, c.`lastname` FROM `exam_bank_folder_authors` AS a
                    JOIN `exam_bank_folder_authors` AS b
