                ...
                switch ($notification_user->getContentType()) {
                    ...
                    case "evaluation_overdue" :
                        ...
                        if ($evaluation) {
                            ...
                            $evaluation_start = date(DEFAULT_DATETIME_FORMAT, $evaluation["evaluation_start"]);
                            $evaluation_finish = date(DEFAULT_DATETIME_FORMAT, $evaluation["evaluation_finish"]);
                            if (isset($evaluation["evaluation_lockout"])) {
                                $evaluation_lockout = date(DEFAULT_DATETIME_FORMAT, $evaluation["evaluation_lockout"]);
                            } else {
                    ...
                    case "assessment" :
                        ...
                        if ($distribution_assessment) {
                            ...
                            $delegator = null;
                            $creator = null;
                            $creator_id = $distribution_assessment->getCreatedBy();
                            if ($distribution_assessment->getADistributionID()) {
                                ...
                                if ($distribution) {
                                    $creator_id = $distribution->getCreatedBy();
                                }
                                if ($delegator && $delegator->getDelegatorType() == "proxy_id" && $delegator->getDelegatorID()) {
                                    $creator_id = $delegator->getDelegatorID();
                                }
                            }
                            if ($creator_id) {
                                $creator = Models_User::fetchRowByID($creator_id);
                            }
                            if ($creator) {
                                $from_email = $creator->getEmail();
                                $from_firstname = $creator->getFirstname();
                                $from_lastname = $creator->getLastname();
                                $assessment_api = new Entrada_Assessments_Assessment(array("dassessment_id" => $distribution_assessment->getID(), "limit_dataset" => array("targets")));
                                $url = $assessment_api->getAssessmentURL(null, null, $assessment_api->isExternal(), $distribution_assessment->getID());
                                $target = array();

                                if ($notification_user->getNotificationUserType() == "proxy_id") {
                                    $internal_target = Models_User::fetchRowByID($proxy_id);
                                    if ($internal_target) {
                                        $target["firstname"] = $internal_target->getFirstname();
                                        $target["lastname"] = $internal_target->getLastname();
                                        $target["email"] = $internal_target->getEmail();
                                        $target["username"] = $internal_target->getUsername();
                                    }
                                } else {
                                    $external_target = Models_Assessments_Distribution_ExternalAssessor::fetchRowByID($proxy_id);
                                    if ($external_target) {
                                        $target["firstname"] = $external_target->getFirstname();
                                        $target["lastname"] = $external_target->getLastname();
                                        $target["email"] = $external_target->getEmail();
                                        $target["username"] = "";
                                    }
                                }
                                ...
                                if (!empty($target)) {
                                    $replace = array(
                                        html_encode($target["firstname"]),
                                        html_encode($target["lastname"]),
                                        html_encode($url),
                                        html_encode($target["username"]),
                                        html_encode($target["email"]),
                                        html_encode(ENTRADA_URL),
                    ...
                    case "assessment_general" :
                        ...
                        if ($build_notification) {
                            if ($distribution = Models_Assessments_Distribution::fetchRowByID($distribution_assessment->getADistributionID())) {
                        ...
                        if ($build_notification) {
                            $creator = Models_User::fetchRowByID($creator_id);
                            if (!$creator) {
                                application_log("error", "Notification failed: Unable to determine who is the notification creator.");
                                $build_notification = false;
                            }
                        }
                        if ($build_notification) {
                    ...
                    case "assessment_approver" :
                        ...
                        if ($progress && $progress->getProgressValue() == "complete") {
                            ...
                            if ($distribution) {
                                ...
                                if ($creator && $approver) {
                                    ...
                                    $url = $assessment_api->getAssessmentURL($target_record_id, "proxy_id", false, $progress->getDAssessmentID());
                        ...
                        break;
                    case "assessment_task_forwarded" :
                        ...
                        if ($distribution_assessment) {
                            $assessment_api = new Entrada_Assessments_Assessment(array("dassessment_id" => $distribution_assessment->getID(), "limit_dataset" => array("targets")));
                            $creator = Models_User::fetchRowByID($distribution_assessment->getForwardedBy());
                            $from_email = $creator->getEmail();
                            $from_firstname = $creator->getFirstname();
                            $from_lastname = $creator->getLastname();
                            ...
                            if ($notification_user->getNotificationUserType() == "proxy_id") {
                                $notification_body = file_get_contents($ENTRADA_TEMPLATE->absolute() . "/email/notification-assessment-new-forwarded.xml");
                            } else {
                                $notification_body = file_get_contents($ENTRADA_TEMPLATE->absolute() . "/email/notification-assessment-external-new-forwarded.xml");
                            }
                            $notification_body = str_replace($search, $replace, $notification_body);
                        }
                        break;
								...
								if ($distribution) {
									...
									if ($creator) {
										...
										$replace = array(
											...
											html_encode(date(DEFAULT_DATETIME_FORMAT, $distribution_assessment->getCreatedDate())),
                    ...
                    case "assessment_removal" :
                        ...
                        break;
                    case "assessment_task_deleted_forwarded" :
                        $search = array("%TARGET_FIRSTNAME%",
                            "%TARGET_LASTNAME%",
                            "%SUBMITTED_BY%",
                            "%URL%",
                            ...
                            "%CREATOR_LASTNAME%",
                            "%CREATOR_EMAIL%");
                        $old_assessment = Models_Assessments_Assessor::fetchRowByID($record_id, null, true);
                        $new_assessment = Models_Assessments_Assessor::fetchRowByID($subcontent_id, null, true);
                        if ($old_assessment && $new_assessment) {
                            $distribution = Models_Assessments_Distribution::fetchRowByID($old_assessment->getADistributionID());
                            if ($distribution) {
                                $creator = User::fetchRowByID($distribution->getCreatedBy());
                            } else {
                                $creator = User::fetchRowByID($old_assessment->getCreatedBy());
                            }
                            $assessor_id = false;
                            $assessor_firstname = "";
                            $assessor_lastname = "";
                            if ($old_assessment->getAssessorType() == "internal") {
                                $assessor_id = $old_assessment->getAssessorValue();
                                ...
                                $external_assessor = Models_Assessments_Distribution_ExternalAssessor::fetchRowByID($old_assessment->getAssessorValue());
                            ...
                            if ($creator && $assessor_id) {
                                ...
                                $from_firstname = $creator->getFirstname();
                                $from_lastname = $creator->getLastname();
                                $submitted_by = Models_User::fetchRowByID($new_assessment->getForwardedBy());
                                $replace = array(
                                    ...
                                    html_encode($creator->getLastname()),
                                    html_encode($creator->getEmail()));
                                $notification_body = file_get_contents($ENTRADA_TEMPLATE->absolute() . "/email/notification-assessment-task-deleted-forwarded.xml");
                                $notification_body = str_replace($search, $replace, $notification_body);
                            }
                        }
                        break;
                    case "assessment_feedback" :
                        ...
                        if ($progress_record) {
                            ...
                            if ($user && $assessor_id) {
                                $assessment_api = new Entrada_Assessments_Assessment(array("limit_dataset" => array("targets")));
                                $url = $assessment_api->getAssessmentURL($proxy_id, "proxy_id", false, $progress_record->getDassessmentID(), $record_id);
                    ...
                    case "assessment_submitted_notify_approver" :
                        ...
                        if ($progress_record) {
                            ...
                            if ($approver_record) {
                                $assessment_api = new Entrada_Assessments_Assessment(array("limit_dataset" => array("targets")));
                                $assessor = $assessment_api->getUserByType($subcontent_id, $notification_user->getNotificationUserType());
                                ...
                                if ($assessor && $approver) {
                                    ...
                                    $url = $assessment_api->getAssessmentURL($progress_record->getTargetRecordID(), "proxy_id", false, $progress_record->getDassessmentID(), $record_id);
                    ...
                    case "assessment_task_deleted" :
                        ...
                        if ($assessment) {
                            $distribution = Models_Assessments_Distribution::fetchRowByID($assessment->getADistributionID());
                            if ($distribution) {
                                $deleted_task = Models_Assessments_AssessmentTarget::fetchRowByID($subcontent_id, time());

                                $assessor_id = false;
                                $assessor_firstname = $assessor_lastname = "";
                                if ($assessment->getAssessorType() == "internal") {
                                    $internal_assessor = Models_User::fetchRowByID($assessment->getAssessorValue());
                                    $assessor_id = $assessment->getAssessorValue();
                                    if ($internal_assessor) {
                                        $assessor_firstname = $internal_assessor->getFirstname();
                                        $assessor_lastname = $internal_assessor->getLastName();
                                ...
                                $creator = User::fetchRowByID($distribution->getCreatedBy());
                                if ($assessor_firstname && $assessor_lastname && $creator && $assessor_id) {
                                    $from_email = $creator->getEmail();
                                    $from_firstname = $creator->getFirstname();
                                    $from_lastname = $creator->getLastname();
                                    $reason = Models_Assessments_TaskDeletedReason::fetchRowByID($deleted_task->getDeletedReasonID());
                                    $deleted_by = User::fetchRowByID($deleted_task->getDeletedBy());
                                    $replace = array(
                                        html_encode($assessor_firstname),
                                        html_encode($assessor_lastname),
                                        html_encode($deleted_by->getFullname(false)),
                                        html_encode($reason ? $reason->getDetails() : ""),
                    ...
                    case "delegation_task_deleted" :
                        ...
                        if ($delegation) {
                            ...
                            if ($distribution) {
                                $delegator_firstname = $delegator_lastname = "";
                                if ($delegation->getDelegatorType() == "proxy_id") {
                                    $internal_assessor = Models_User::fetchRowByID($delegation->getDelegatorID());
                                    if ($internal_assessor) {
                                        $delegator_firstname = $internal_assessor->getFirstname();
                                        $delegator_lastname = $internal_assessor->getLastName();
                                    }
                                } else {
                                    $external_assessor = Models_Assessments_Distribution_ExternalAssessor::fetchRowByID($delegation->getDelegatorID());
                                    if ($external_assessor) {
                                        $delegator_firstname = $external_assessor->getFirstname();
                                        $delegator_lastname = $external_assessor->getLastname();
                                    }
                                }

                                $creator = Models_User::fetchRowByID($distribution->getCreatedBy());
                                $submitted_by = Models_User::fetchRowByID($notification_user->getProxyID());
                                $reason = Models_Assessments_TaskDeletedReason::fetchRowByID($delegation->getDeletedReasonID());

                                if ($creator && $reason && $submitted_by) {
                                    $from_email = $creator->getEmail();
                                    $from_firstname = $creator->getFirstname();
                                    $from_lastname = $creator->getLastname();
                                    $replace = array(
                                        html_encode($delegator_firstname),
                                        html_encode($delegator_lastname),
                                        html_encode($submitted_by->getFirstname() . " " . $submitted_by->getLastname()),
                                        html_encode(ENTRADA_URL . "/assessments"),
                                        html_encode((isset($reason) && $reason ? $reason->getDetails() : "")),
                                        html_encode($delegation->getDeletedReasonNotes() ? $delegation->getDeletedReasonNotes() : ""),
                                        html_encode(ENTRADA_URL),
                                        html_encode($creator->getFirstname()),
                                        html_encode($creator->getLastname()),
                                        html_encode($creator->getEmail()));
                    ...
                    case "assessment_flagged_response" :
                        ...
                        if ($progress_record) {
                            ...
                            $content_title = "N/A";
                            if ($distribution) {
                                $content_title = $distribution->getTitle();
                            }
                            ...
                            // Using the assessment API, fetch a limited dataset (only containing the targets)
                            $assessment_api = new Entrada_Assessments_Assessment(
                                array(
                                    "dassessment_id" => $progress_record->getDassessmentID(),
                                    "aprogress_id" => $progress_record->getAprogressID(),
                                    "limit_dataset" => array("targets")
                                )
                            );
                            // Grab the target, and use that name
                            $assessment_data = $assessment_api->fetchAssessmentData();
                            $assessment_target = $assessment_api->getCurrentAssessmentTarget($progress_record->getTargetRecordID(), $progress_record->getTargetType());
                            if (!empty($assessment_target)) {
                                $target_name = $assessment_target["target_name"];
                            } else {
                                $target_name = "N/A";
                            }
                            if (!$distribution) {
                                // Use the form name instead. Fetch from the forms API (limited dataset)
                                $forms_api = new Entrada_Assessments_Forms(array("form_id" => $assessment_data["assessment"]["form_id"]));
                                $forms_api->setFormDatasetLimit(array("form"), false);
                                $form_data = $forms_api->fetchFormData();
                                if (!empty($form_data)) {
                                    $content_title = $form_data["form"]["title"];
                                }
                            }
                            $url = $assessment_api->getAssessmentURL($proxy_id, "proxy_id", false, $progress_record->getDassessmentID(), $record_id);
                            if ($assessor_id) {
                        ...
                        break;
                    case "assessment_expiry_warning" :
                        $search = array(
                            "%TARGET_FIRSTNAME%",
                            "%TARGET_LASTNAME%",
                            "%DATE%",
                        ...
                        if ($distribution_assessment) {
                            ...
                            if ($creator) {
                                ...
                                $date = date("Y-m-d h:i A", $distribution_assessment->getExpiryDate());
                                ...
                                if (!empty($target)) {
                                    ...
                                    $notification_body = file_get_contents($ENTRADA_TEMPLATE->absolute() . "/email/notification-assessment-expiry-warning.xml");
                                    $notification_body = str_replace($search, $replace, $notification_body);
                                }
                            }
                        }
                    break;
                    case "preceptor_access_request" :
                        $user = Models_User::fetchRowByID($proxy_id);
                        $requesting_user = array();
                        $access_request_model = new Models_User_Access_Request();
                        $access_request_record = $access_request_model->fetchRowByID($record_id);

                        if ($access_request_record) {
                            $requesting_user = Models_User::fetchRowByID($access_request_record->getCreatedBy());
                        }

                        if ($user && $access_request_record && $requesting_user) {
                            $search = array(
                                "%TARGET_FIRSTNAME%",
                                "%TARGET_LASTNAME%",
                                "%LEARNER_FIRSTNAME%",
                                ...
                                "%GROUP%",
                                "%ROLE%",
                                "%MISSING_PRECEPTOR_FIRSTNAME%",
                                "%MISSING_PRECEPTOR_LASTNAME%",
                                "%MISSING_PRECEPTOR_EMAIL%",
                                "%MISSING_PRECEPTOR_NUMBER%",
                                "%ADDITIONAL_COMMENTS%",
                            ...
                            $replace = array(
                                $user->getFirstname(),
                                $user->getLastname(),
                                $requesting_user->getFirstname(),
                                $requesting_user->getLastname(),
                                $access_request_record->getRequestedGroup(),
                                $access_request_record->getRequestedRole(),
                                $access_request_record->getRequestedUserFirstname(),
                                $access_request_record->getRequestedUserLastname(),
                                $access_request_record->getRequestedUserEmail(),
                                ($access_request_record->getRequestedUserNumber() ? $access_request_record->getRequestedUserNumber() : "Not provided"),
                                ($access_request_record->getAdditionalComments() ? $access_request_record->getAdditionalComments() : "Not provided"),
                                "healthsci.support@queensu.ca",
                                APPLICATION_NAME,
                                ENTRADA_URL,
                            );

                            $notification_body = file_get_contents($ENTRADA_TEMPLATE->absolute() . "/email/notification-user-access-request.xml");
                            $notification_body = str_replace($search, $replace, $notification_body);
                        }
                        break;
                    case "preceptor_inactive_access_request" :
                        $user = Models_User::fetchRowByID($proxy_id);
                        $requesting_user = array();
                        $access_request_model = new Models_User_Access_Request();
                        $access_request_record = $access_request_model->fetchRowByID($record_id);
                        ...
                        if ($user && $access_request_record && $requesting_user) {
                            ...
                            $notification_body = file_get_contents($ENTRADA_TEMPLATE->absolute() . "/email/notification-user-inactive-access-request.xml");
                            $notification_body = str_replace($search, $replace, $notification_body);
                        }
                        break;
                    default :
