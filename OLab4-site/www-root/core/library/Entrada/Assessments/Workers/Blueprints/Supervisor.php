 ...
 *
 * Abstraction layer for Supervisor Blueprint form generation.
 ...
 *
 */
class Entrada_Assessments_Workers_Blueprints_Supervisor extends Entrada_Assessments_Workers_Blueprints_Base {
    ...
    public function publish($related_dataset) {
        ...
        $cbme_form_title = isset($related_dataset["form_blueprint"]["title"]) ? $related_dataset["form_blueprint"]["title"] : $translate->_("Supervisor Assessment");
        ...
        foreach ($forms as $form_id) {
            ...
            if (isset($milestones[$epa_objective->getID()]) && is_array($milestones[$epa_objective->getID()])) {
                foreach ($milestones[$epa_objective->getID()] as $milestone) {
                    $objectives[] = $milestone["objective_id"];
                }
            }
            ...
            foreach ($related_dataset["components"] as $component_order => $component) {
                ...
                switch ($component["shortname"]) {
                    ...
                    case "entrustment_scale":
                        ...
                        $item_id = $this->addItemFromScale($related_dataset["scales"][$element["afblueprint_element_id"]], $element, $objectives, "CBME-supervisor-entrustment_scale");
                        if ($item_id === false) {
                            return false;
                        }
                        ...
                        break;
                    case "ms_ec_scale":
                        $rubric_ids = $this->addRubricFromScale($epas[$form_id], $related_dataset["scales"][$element["afblueprint_element_id"]], $milestones, $element, array("reorderable_in_form" => true));
                        if (!empty($rubric_ids)) {
                            foreach ($rubric_ids as $rubric_id) {
                                $form->attachRubric($rubric_id);
                            }
                        }
                        break;
    ...
    /**
     * Implementation of abstract getComponents(). Returns the components for a supervisor form.
     *
     * @return array|bool
     */
    public function getComponents() {
        ...
        // A supervisor form is defined as having these components (in order):
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("epa_selector");
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("contextual_variable_list");
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("ms_ec_scale");
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("entrustment_scale");
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("standard_item");
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("standard_item");
        $fetched[] = Models_Assessments_Form_Blueprint_Component::fetchRowByShortname("standard_item");
     ...
     *
     * For the Supervisor blueprint, we must update 4 different types of component types, of which, three use base-class methods.
     *
    ...
    public function updateComponentProgress($component_progress_data) {
        ...
        switch ($component_progress_data["blueprint_component_type"]) {
            ...
            case "ms_ec_scale":
                return $this->updateBlueprintElementScale($this->form_blueprint_id, "milestone_ec", "ms_ec_scale", $component_progress_data["components"], $component_progress_data["component_data"], 2, $component_progress_data["comment_type"], $component_progress_data["flagged_responses"], $component_progress_data["default"]);
            case "entrustment_scale":
                return $this->updateBlueprintSupervisorEntrustmentScale($this->form_blueprint_id, $component_progress_data["components"], $component_progress_data["component_data"], 3, $component_progress_data["comment_type"], $component_progress_data["flagged_responses"], $component_progress_data["editor_state"], $component_progress_data["default"]); // $editor_state in this case is a string of plain text (the item text for the item this scale is attached to).
            default:
    ...
    protected function updateBlueprintElementContextualVariableList($form_blueprint_id, $cv_arrays, $components, $component_order) {
        ...
        if (!empty($validation_failures)) {
            foreach ($validation_failures as $epa_objective_id => $failure_tokens) {
                if ($epa_objective = $this->fetchObjectiveRecordByID($epa_objective_id)) { // local cache
                    ...
                    $this->addErrorMessage($translate->_("Unable to fetch EPA ($epa_objective_id)."));
                }
            }
            return false;
        }
    ...
    /**
     * Initialize the supervisor form by building the standard items.
     *
     * @return bool
     */
    ...
    /**
     * For a supervisor form, add an entrustment scale item, with given item text, for a specified scale.
     *
     ...
     * @return bool
     */
    private function updateBlueprintSupervisorEntrustmentScale($form_blueprint_id, $components, $component_data, $component_order, $comment_type, $flagged_responses, $item_text, $default = null) {
        global $translate;
