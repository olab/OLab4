...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 onChangeSort: false,
			 onSuccess: false,
			 collapseTable: true, // Modified by Andrew Dos-Santos to allow for grids to minimized by default. Changes made at line 1237 to allow for this.
			 disableSelect: false, // Modified by Andrew Dos-Santos to allow for disabling row select. Changes made at line 730 to allow for this.
		...
		if (p.title)
		{
			...
			if (p.showTableToggleBtn)
			{
				if(p.collapseTable) { 
					...
					$(g.mDiv).append('<div class="ptogtitle vsble" title="Minimize/Maximize Table"><span></span></div>');
				} else {
