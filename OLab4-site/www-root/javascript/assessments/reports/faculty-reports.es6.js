...
jQuery(function($) {
    ...
    $("#select_course_btn").advancedSearch({
        api_url : ENTRADA_URL + "/admin/assessments?section=api-assessment-reports",
    ...
    $("#select_faculty_btn").advancedSearch({
        api_url : ENTRADA_URL + "/admin/assessments?section=api-assessment-reports",
        resource_url: ENTRADA_URL,
        filters : {
            faculty : {
    ...
    $("#select_distribution_btn").advancedSearch({
        ...
        no_results_text: "No distributions found matching the search criteria.",
        parent_form: $("#report-form"),
        control_class: "form-selector",
        width: 350,
        select_all_enabled : true
...
jQuery(document).ready(function ($) {
    ...
    /* This report can be POSTed to with preset courses, faculty, etc. To accommodate this, we check if any
    requirements have been met immediately on page load. */
    handleChange("course");
    ...
    $("#generate-pdf-btn").on("click", function (e) {
        ...
        if (parameters_string) {
            window.location = ENTRADA_URL + "/admin/assessments?section=api-assessment-reports&method=generate-bulk-pdf-reports" + parameters_string;
        }
    });
    ...
    function requirementsMet(control) {
        switch (control) {
            ...
            case "end_date":
                ...
                console.log($("input[name=\"course[]\"]").length);
                ...
                break;
            case "faculty":
                // We must have selected faculty to proceed.
                if ($("input[name=\"faculty[]\"]").length) {
                    return true;
                }
                break;
            case "form":
    ...
    function configureDependantControls(control) {
        ...
        switch (control) {
            case "course":
                ...
                setAdvancedSearchApiParameter("distribution", "distribution", "course_ids", course_ids);
            ...
            case "faculty":
                ...
                let faculty_ids = [];
                $("input[name=\"faculty[]\"]").each(function (i, v) {
                    if ($(v).val() > 0) {
                        faculty_ids.push($(v).val());
                    }
                });
                ...
                $(control_flow[control].next).each(function (i, next_control) {
                    if (next_control in control_flow) {
                        setAdvancedSearchApiParameter(next_control, next_control, "target_ids", faculty_ids);
                        // End date will be applied all the way up.
                        configureDependantControls(next_control);
                    }
                });
                ...
                // Faculty are the final targets for this report.
                submission_values["target_ids"] = faculty_ids;

                if (faculty_ids.length < 1) {
                    hideNext(control);
                }
                break;
    ...
    function setAdvancedSearchApiParameter(control, filter, parameter_name, parameter_value) {
        console.log($("#select_" + control + "_btn"));
        console.log($("#select_" + control + "_btn").data());
