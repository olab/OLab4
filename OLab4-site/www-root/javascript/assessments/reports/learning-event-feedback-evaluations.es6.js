let control_flow = {
    ...
    "include_event_info_subheader": {
        control: "include_event_info_subheader"
    },
    "include_comments": {
    ...
    "generate": {
        ...
        api_params: {                               // API params will be stored over the course of data selection for final submission.
            "target_type": "event_id",              // Target type is event.
            "include_comments": true,               // Page defaults to having comments enabled.
            "include_event_info_subheader": true,   // Page defaults to having event subheader enabled.
            "is_evaluation": true
        }
    }
};
...
jQuery(document).ready(function ($) {
    ...
    $("#generate-pdf-btn").on("click", function (e) {
        ...
        if (parameters_string) {
            window.location = ENTRADA_URL + "/admin/assessments?section=api-assessment-reports&method=generate-bulk-event-feedback-pdf-reports" + parameters_string;
        }
    });
    ...
    function handleChange(control) {
        switch (control) {
            case "include_event_info_subheader":
    ...
    function requirementsMet(control) {
        switch (control) {
            ...
            case "end_date":
            case "eventtype":
                // Course, event types, start date and/or end date must be set to proceed.
                if ($("input[name=\"course[]\"]").length &&
                    $("input[name=\"eventtype[]\"]").length &&
    ...
    function configureDependantControls(control) {
        ...
        switch (control) {
            case "course":
                ...
                // Target type is event.
                setAdvancedSearchApiParameter("form", "form", "target_type", "event_id");
                // All feedback forms are distributionless.
                setAdvancedSearchApiParameter("form", "form", "distributionless", true);
            ...
            case "eventtype":
                let eventtype_ids = [];
                $("input[name=\"eventtype[]\"]").each(function (i, v) {
                    if ($(v).val() > 0) {
                        eventtype_ids.push($(v).val());
                    }
                });

                // Event type influence almost everything.
                // Set as an API parameter on dependant controls.
                $(control_flow[control].next).each(function (i, next_control) {
                    if (next_control in control_flow) {
                        setAdvancedSearchApiParameter(next_control, next_control, "eventtype_ids", eventtype_ids);
                        // Event type will be applied all the way up.
                        configureDependantControls(next_control);
                    }
                });

                submission_values["eventtype_ids"] = eventtype_ids;

                if (eventtype_ids.length < 1) {
                    hideNext(control);
                }
                break;
            ...
            case "form":
                ...
                setAdvancedSearchApiParameter("event", "event", "form_ids", form_ids);
            ...
            case "event":
                ...
                $("input[name=\"event[]\"]").each(function (i, v) {
                    if ($(v).val() > 0) {
                        event_ids.push($(v).val());
                    }
                });

                submission_values["event_ids"] = event_ids;
                break;

            case "individual_event_option":
                is_checked = !!$("#individual_event_option").is(":checked");
                submission_values["individual_event_option"] = is_checked;
                break;

            case "include_event_info_subheader":
                is_checked = !!$("#include_event_info_subheader").is(":checked");
                submission_values["include_event_info_subheader"] = is_checked;
                if (!is_checked) {
                    hideNext(control)
                }
                break;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (jsonResponse.status === "success") {
                    ...
                    setAdvancedSearchApiParameter("event", "event", "start_date", start_date);
                    setAdvancedSearchApiParameter("event", "event", "end_date", end_date);
                }
            }
        });
    }
