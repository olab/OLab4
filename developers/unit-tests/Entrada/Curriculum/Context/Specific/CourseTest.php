...
class Entrada_Curriculum_Context_Specific_CourseTest extends BaseTestCase {
    ...
    public function testGetEventIDs() {
        $context = Phake::partialMock("Entrada_Curriculum_Context_Specific_Course", 1);
        $event = new Models_Event(array("event_id" => 101, "cunit_id" => 11, "course_id" => 1));
        Phake::when($this->event_repository)->fetchAllByCourseIDs(Phake::anyParameters())->thenReturn(array($event));
        $this->assertEmpty(Phake::makeVisible($context)->getEventIDs());
        Phake::verify($this->event_repository, Phake::never())->fetchAllByCourseIDs(Phake::anyParameters());
    }

    public function testGetCunitIDs() {
        $context = Phake::partialMock("Entrada_Curriculum_Context_Specific_Course", 1);
        $course_unit = new Models_Course_Unit(array("cunit_id" => 11, "course_id" => 1));
        Phake::when($this->course_unit_repository)->fetchAllByCourseIDs(Phake::anyParameters())->thenReturn(array($course_unit));
        $this->assertEmpty(Phake::makeVisible($context)->getCunitIDs());
        Phake::verify($this->course_unit_repository, Phake::never())->fetchAllByCourseIDs(Phake::anyParameters());
    }
...
if (!defined('PHPUnit_MAIN_METHOD')) {
    Entrada_Curriculum_Context_Specific_CourseTest::main();
}
